...
fireon_path = file.path(main_path,"fireon-site")
...
param_file  = file.path(main_path,"fates_Fireonparams.nc")
#obs path
...
#fwrite(byage,file.path(main_path,paste0("allcaseFireon_byage.csv")))
#fwrite(bypft,file.path(main_path,paste0("allcaseFireon_bypft.csv")))
#fwrite(dphen,file.path(main_path,paste0("allcaseFireon_dphen.csv")))
#fwrite(bysoi,file.path(main_path,paste0("allcaseFireon_bysoi.csv")))
#fwrite(hlm1d,file.path(main_path,paste0("allcaseFireon_hlm1d.csv")))

bypft = fread(file.path(main_path,"allcaseFireon_bypft.csv"))
hlm1d = fread(file.path(main_path,"allcaseFireon_hlm1d.csv"))
...
p_vlist  = param_config$uniq_name[grepl(pattern="fire",param_config$uniq_name)]
np       = length(p_vlist)
...
```{r,label="plot-monthly-mean-fire"}
...
mmean_hlm      = hlm1d                                                      %>% 
              dplyr::select(all_of(
                c("case","ens_label","time","eflx_lh_tot",fire_vars)))      %>% 
              mutate(month=month(time))                                     %>% 
              select(-time)                                                 %>% 
              group_by(case,ens_label,month)                                %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
...
pars         = names(param_survi)[c(1:5)] #slamax and slatop are same, so we chose only one
n_param      = length(pars)
...
type        = "fire season"
cor_survi   = cor_survi[c(1,3:20)] 
cor_allens  = cor_allens[c(1,3:20)]
...
for(c in sequence(ncase)){
  ...
  cor_nams          = c("GPP","$ Mort[Cstar]","$ Mort[Hydro]","$ Mort[Term]",
                        "$ Mort[Can]","$ Mort[Und]","AGB","LAI",":lambda*E",
                        "$ Fire[Area]","$ Fire[FDI]", "$ Fire[Fuel]",
                        "$ Fire[Fblkd]", "$ Param[FBD-dead]","$ Param[FBD-live]",
                        "$ Param[Nignitn]", "$ Param[Drying]", "$ Param[fuel-enrgy]")
...
decom_y = c("fates_burnfrac")
decom_x = names(fssur_sens)[c(3:10,13:20)]
n_x     = length(decom_x)
n_y     = length(decom_y)
...
keep_region = unique(filt_fire$case_ens) #there are 84 ensembles 
...
# min and max cell-level mean annual burned fraction
lw_bf       = min(cags_dmean$bfrac)
hi_bf       = max(cags_dmean$bfrac)
...
filt_fire    = filt_fire                                         %>% 
               ...
               summarize(bfrac_model = 
                           mean(fates_burnfrac,na.rm=TRUE))      %>% 
               ungroup()
filt_fire$lw_bf = lw_bf
filt_fire$hi_bf = hi_bf


## filter with low and max mean annual bf

filt_fire     = filt_fire                                         %>% 
                filter(bfrac_model >=lw_bf & bfrac_model<=hi_bf)  
keep_region1 = filt_fire$case_ens[order(filt_fire$bfrac_model,decreasing=TRUE)[1:5]]
...
keep_region2 = unique(filt_fire$case_ens) #45 ensemble members
...
keep_final  = c(keep_region1,keep_region2,keep_region3)
...
for(h in fire_loop){
  ...
  fire_now        = fireamean_keep
   ...
   gg_now = ggplot(data=fire_now)
   gg_now = gg_now + facet_wrap(.~case,labeller=labeller(case=case_desc))
   gg_now = gg_now + geom_line(aes_string(x="year",y=h_vnam,group="ens_label",color="'Model'"),lwd=0.2,alpha=0.8,show.legend=TRUE)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(h_vnam,"_combineKeep.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_now
                    , device    = gg_device[d]
                    , path      = secy_path
