...
ens_cases  = c(  "avba-fireon-400-g1.nc"
               , "avba-fireon-400-g2.nc"
               , "brdi-fireon-400-g1.nc"
               , "brdi-fireon-400-g2.nc"
               , "brdi-fireon-400-g3.nc"
               , "genl-fireon-400-g1.nc"
               , "genl-fireon-400-g2.nc"
               , "genl-fireon-400-g3.nc")

case_files   = file.path(fireon_path,ens_cases)
ncase       = length(ens_cases)
case_names  = c(1L,2L,3L,4L,5L,6L,7L,8L)

case_desc   = c( "AVBA-Fire-Base1"
               , "AVBA-Fire-Base2"
               , "BRDI-Fire-Base1"
               , "BRDI-Fire-Base2"
               , "BRDI-Fire-Base3"
...
names(case_desc) = c("1","2","3","4","5","6","7","8")

trait_file  = file.path(main_path,"global-varying-params.csv")
param_file  = file.path(fireon_path,"fates_site_fireon.nc")
#obs path
...
frap_grass_wrf  = file.path(frap_main,"Bfrac-WRF-grassonly_FRAP.csv")
#frap_grass  = file.path(frap_main,"Burnedfractionbycell.csv")
frap_site   = file.path(frap_main,"site-nearest-fire.csv")
frap_raw    = file.path(obs_main,"CA-grassfire-climate/fire20_1.gdb")
...
#plot path
plot_main  = file.path(fireon_path,"Figures",case_desc[1])
...
node_ens = node_ens[-c(501:len_node)]
...
#--- Create molten data table for this variable.  
  ...
  if (is.null(now_fuelmelt)){
    now_fuelmelt = now_melt
        ...
        now_fuelmelt = as_tibble(merge(x=now_fuelmelt,y=now_melt,by=c("case","ens_label","time","fuel"),all=TRUE))
  }#end if (is.null(age_melt))
...
#fwrite(byage,file.path(fireon_path,paste0("allcaseFireon_byage.csv")))
#fwrite(bypft,file.path(fireon_path,paste0("allcaseFireon_bypft.csv")))
#fwrite(dphen,file.path(fireon_path,paste0("allcaseFireon_dphen.csv")))
#fwrite(bysoi,file.path(fireon_path,paste0("allcaseFireon_bysoi.csv")))
#fwrite(hlm1d,file.path(fireon_path,paste0("allcaseFireon_hlm1d.csv")))
#fwrite(byfuel,file.path(fireon_path,paste0("allcaseFireon_byfuel.csv")))

bypft  = fread(file.path(fireon_path,"allcaseFireon_bypft.csv"))
hlm1d  = fread(file.path(fireon_path,"allcaseFireon_hlm1d.csv"))
byfuel = fread(file.path(fireon_path,"allcaseFireon_byfuel.csv"))
...
```{r,label="retrive-global-params-for-ensembles", results='hide'}
...
p_fire    = param_config %>% filter(experiment %in% c("both","fire_on"))
p_vlist   = p_fire$uniq_name
...
for (p in sequence(np)){
  ...
  nc_nvnow            = p_vlist[p]
  h                   = match(nc_nvnow,param_config$uniq_name)
  fates_p             = param_config$fates_parameter[h]
  ...
  p_value             = ncvar_get(nc=nc_conn,varid=fates_p)
 ...
 p_name    = paste0(o_prefix,"_",fates_p)
 param_lst[[p]] = setNames(as_tibble(p_value[p_org_idx,]),p_name)
 
 }else if (is_pft){
 p_prefix  = sprintf("p%i",p_pft_idx)
 p_name    = paste0(p_prefix,"_",fates_p)  
...
plot_names = gsub(pattern="fates_fire_",replacement="",x=names(param_survi))
...
for (c in sequence(ncase)){
  ...
  Hmisc::hist.data.frame(survi_his[c(1:5)],mtitl = paste0(desc_now,"_survied"))
  dev.off()
  
  filename=paste0(desc_now,"_paramHist-dead.png")
  png(file=file.path(plot_main,filename)
     ,width=10,height=8,units="in",res=600)
  Hmisc::hist.data.frame(dead_his[c(1:5)] ,mtitl = paste0(desc_now,"_dead"))
}
...
for (c in sequence(ncase)){
  ...
  png(file=file.path(plot_main,filename)
     ,width=10,height=8,units="in",res=600)
  Hmisc::hist.data.frame(hist_df[6:19],mtitl = paste(desc_now,"monthly mean",sep=" "))
  dev.off()
}
...
fire_cmean = hlm1d                                                 %>% 
             select(all_of(c("case","time",fire_vars)))            %>% 
             mutate(month = month(time))                           %>% 
             group_by(case,month)                                  %>% 
             dplyr::select(-time)                                  %>% 
             summarize_all(mean,na.rm=TRUE)                        %>% 
...
##1. observed seasonal variations in burned area of lightning-caused fires in California
...
# 2. Observed monthly burned fraction across CA grasslands 
bf_grass  = fread(frap_grass_wrf)
bfam_grass  = bf_grass                                               %>% 
            mutate(case=0)                                           %>% 
            group_by(case,Month)                                     %>% 
            summarize(fates_burnfrac = sum(BurnedFrac,na.rm=TRUE))   %>% 
                     #,fates_burnfrac_qupr = quantile(BurnedFrac,probs=qupr_ribbon,names=FALSE, na.rm=TRUE)
                     #,fates_burnfrac_qlwr = quantile(BurnedFrac,probs=qlwr_ribbon,names=FALSE, na.rm=TRUE)) %>% 
            mutate(fates_burnfrac = fates_burnfrac/749)              %>% 
            ungroup() %>% rename(month = Month)
grass_fire = fire_cmean %>% dplyr::select(case,month,fates_burnfrac,fates_burnfrac_qupr,fates_burnfrac_qlwr) 
grass_fire = rbind(bfam_grass,grass_fire) %>% arrange(case,month)
...
for(h in fire_loop){
   ...
   gg_now = gg_now + geom_line(data=bfam_grass,aes(y=fates_burnfrac / tf),
...
```{r,label="plot-allometry-effects",message=FALSE,fig.width=8,fig.height=10}

fire_month = c("6","7","8","9","10")

fire_allom = hlm1d                                                 %>% 
             select(c("case","ens_label","time",fire_vars))        %>% 
             mutate(case_ens = paste0(case,"_",ens_label)
                   , month = month(time))                          %>% 
...
for(h in fire_loop){
  ...
  fire_now$case = factor(fire_now$case,levels=unique(fire_now$case))
   ...
   gg_now = gg_now + geom_boxplot(aes_string(x="case",y=h_vnam,color="case")
                                 ,show.legend=TRUE)
   
   gg_now = gg_now + scale_colour_manual(name=""
                                        ,labels=case_desc
                                        ,values=top12)
   if( h_vnam == "fates_fuel_amount"){
     gg_now = gg_now + geom_hline(aes(yintercept = 0.6),color = "black",linetype='dashed')
     gg_now = gg_now + geom_hline(aes(yintercept = 0.26), color="black",linetype="dashed")
     #gg_now = gg_now + geom_rect(aes(xmin =-Inf, xmax=Inf, x=NULL, ymin=0.26,ymax=0.6),alpha=0.1,fill="grey")
   }
   gg_now = gg_now + labs(title=h_desc)
   gg_now = gg_now + xlab(element_blank())
   gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(h_vnam,"_allomgroup.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_now
                    , device    = gg_device[d]
                    , path      = secy_path
...
mmean_hlm      = hlm1d                                                      %>% 
              dplyr::select(all_of(c("case","ens_label","time","eflx_lh_tot",fire_vars)))      %>% 
              mutate(month=month(time))                                     %>% 
              select(-time)                                                 %>% 
              group_by(case,ens_label,month)                                %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
...
max_hlmall    = mmean_hlm                                                   %>% 
               group_by(case,ens_label)                                     %>% 
               select(-c(month,case_ens))                                   %>% 
               summarize_all(max,na.rm=TRUE)                                %>% 
               ungroup()
max_hlmsur    = mmean_hlm                                                   %>% 
               filter(case_ens %in% survi_label)                            %>% 
               group_by(case,ens_label)                                     %>% 
               select(-c(month,case_ens))                                   %>% 
               summarize_all(max,na.rm=TRUE)                                %>% 
...
maxsur_sens     = max_survi                                     %>% 
                  left_join(max_hlmsur,by=c("case","ens_label"))
maxall_sens     = max_all                                       %>% 
                  left_join(max_hlmall,by=c("case","ens_label"))

#2. fire variables and mortality only in fire season: 6,7,8,9,10, lai, gpp, agb in growing season 3,4,5
mort    = asses_vars[2:6]
produc  = asses_vars[c(1,7:8)]

mort_sur   =    survi_mmean                                                   %>% 
                filter(month %in% fire_month)                                 %>% 
                dplyr::select(all_of(c("case","ens_label",mort)))             %>% 
                group_by(case,ens_label)                                      %>% 
                summarize_all(mean,na.rm=TRUE)                                %>% 
                ungroup()
prod_sur   =    survi_mmean                                                   %>% 
                filter(month %in% growing_sea)                                %>% 
                dplyr::select(all_of(c("case","ens_label",produc)))           %>% 
                group_by(case,ens_label)                                      %>% 
                summarize_all(mean,na.rm=TRUE)                                %>% 
                ungroup()
fs_sur     = prod_sur %>% left_join(mort_sur,by=c("case","ens_label"))
...
fs_all     = prod_all %>% left_join(mort_all,by=c("case","ens_label"))

fs_fireall    = mmean_hlm                                                   %>% 
                filter(month %in% fire_month)                               %>% 
                group_by(case,ens_label)                                    %>% 
                select(-c(month,case_ens))                                  %>% 
                summarize_all(mean,na.rm=TRUE)                              %>% 
                ungroup()
fs_firesur    = mmean_hlm                                                   %>% 
                filter(case_ens %in% survi_label & month %in% fire_month)   %>% 
                group_by(case,ens_label)                                    %>% 
                select(-c(month,case_ens))                                  %>% 
                summarize_all(mean,na.rm=TRUE)                              %>% 
                ungroup()
fsall_sens  = fs_all                                        %>% 
              left_join(fs_fireall, by=c("case","ens_label"))
fssur_sens  = fs_sur                                        %>% 
              left_join(fs_firesur, by=c("case","ens_label"))


decomp_vars   = c (fire_vars,asses_vars)
n_var         = length(asses_vars)

pars         = names(param_survi)[c(1:6)] #slamax and slatop are same, so we chose only one
n_param      = length(pars)
...
fssur_sens   = fssur_sens                                        %>% 
               left_join(param_survi,by=c("case","ens_label"))
fsall_sens   = fsall_sens                                        %>% 
               left_join(param_all,by=c("case","ens_label"))
...
```{r,label="trait-model-ouputs-correlations",message=FALSE,fig.height=12, fig.width=14}


cor_survi   = fssur_sens
cor_allens  = fsall_sens
type        = "all season"
#cor_survi   = cor_survi[c(1,3:15,17:22)] 
cor_allens  = cor_allens[,c(1,3:15,17:22)]
cor_nams    = c("GPP","AGB","LAI","$ Mort[Cstar]","$ Mort[Hydro]","$ Mort[Term]",
                "$ Mort[Can]","$ Mort[Und]","LH",
                "$ Fire[Area]","$ Fire[FDI]","$ Fire[ROS]", "$ Fire[Fuel]",
                "$ Param[FBD-dead]","$ Param[FBD-live]",
                "$ Param[Nignition]", "$ Param[Drying]", "$ Param[fuel-enrgy]",
                "$ Param[Frag]")
...
for(c in sequence(ncase)){
 ...
 #             dplyr::select(-case)
  ...
  f_output = paste0(desc_now,"_",type,"_correlation-all.png")
  ...
  corrplot(corr_allens
          ...
          ,insig       = "blank"
          #,addCoef.col = "grey50"
          #,title = paste(desc_now,type,"all ensembles",sep=" ")
          ,title=desc_now
          ,mar=c(0,0,1,0)
 ...
 # f_output=paste0(desc_now,"_",type,"_correlation-survi.png")
 # png(file=file.path(plot_main,f_output),
 ...
 #         ,insig       = "blank"
 #         ,addCoef.col = "grey50"
 #         ,title       = paste(desc_now,type,"survived ensembles",sep=" ")
 #         ,mar         = c(0,0,1,0)
 #         ,order       = "original"
          #,diag  = FALSE
 #         ,number.cex  = 0.55
...
# by case
if_plot=FALSE
decom_y = c("fates_burnfrac","fates_fuel_amount")
decom_x = names(fssur_sens)[c(3:11,13:15,17:22)]
n_x     = length(decom_x)
n_y     = length(decom_y)
sens_main = matrix( data=NA_real_
                  , nrow=n_y*ncase
                  , ncol=n_x+1
                  , dimnames = list(rep(decom_y,time=ncase),c(decom_x,"case")))

last_col = ncol(sens_main)
sens_plot = list()
sens_df   = fssur_sens #to select doing variance decompose for all or survived ensembles
type      = "fire-season-bycase"
for(c in sequence(ncase)){
  case_now  = case_names[c]
  desc_now  = case_desc[c]
  sens_now  = sens_df   %>%  filter(case==case_now)
  for(v in sequence(n_y)){
  var_now = decom_y[v]
  for(p in sequence(n_x)){
    p_now   = decom_x[p] 
...
decomall_y1 = c("fates_burnfrac")
decomall_x1 = names(fssur_sens)[c(1,17:22)] #only global varying parameter
decomall_y2 = c("fates_fuel_amount")
decomall_x2 = names(fssur_sens)[c(1,3:10,22)] #global varying parameter and variable
decomall_y    = decomall_y1
decomall_x = decomall_x1
n_x     = length(decomall_x)
n_y     = length(decomall_y)
sensall_main = matrix( data=NA_real_
                  , nrow=n_y
                  , ncol=n_x
                  , dimnames = list(decomall_y,decomall_x))
sens_plot = list()
sens_now   = fssur_sens %>%  mutate(case = as.factor(case))
type      = "fire-survi-allcase"

  for(v in sequence(n_y)){
  var_now = decomall_y[v]
  for(p in sequence(n_x)){
    p_now   = decomall_x[p] 
    df_now  = sens_now %>% select(all_of(c(p_now,var_now)))
    x       = df_now[[p_now]]
    y       = df_now[[var_now]]
    fit     = lm(y ~ x)
    moddf      = data.frame(x=x,y=y)
  ...
  sens_plot[[var_now]][[p_now]]=gg_now } #end of if(if_plot)
...
sensall_bf = sensall_main
sensall_fl = sensall_main
...
sens_madf$type       = "main"
sens_decom           = sens_madf

sens_decom           = sens_decom                                              %>%  
                       pivot_longer(cols = decom_x[1]:decom_x[length(decom_x)]
                                   ,names_to  = "parameter"
                                   ,values_to = "var_decomp")   
par_label            = cor_nams[c(1:9,11:19)]
par_label            = gsub(pattern="\\$ ", replacement="",x=par_label)
par_label            = gsub(pattern=":", replacement="", x=par_label)

sens_decom           = sens_decom                                                %>% 
                       mutate( var_decomp = round(as.numeric(var_decomp)*100,1)
                             , label      = rep(par_label,time=n_y*ncase/2)) 
  
var_label            = c("Burned area"
                        ,"Fire danger index"
                        ,"Fuel amount"
                        ,"Fuel bulk density")

names(var_label)     = c("fates_burnfrac"
                        ,"fates_fdi"
                        ,"fates_fuel_amount"
                        ,"fates_fuel_bulkd")
...
for(c in sequence(ncase)){
  ...
  gg_now = gg_now + geom_linerange(aes(xmin=0,xmax=var_decomp)
                                  ,size=5)#,position = position_dodge(width = 0.2))
  ...
  gg_now = gg_now + labs(x="Variance explained (%)",y="",title=desc_now)
  gg_now = gg_now + scale_y_discrete(labels = scales::label_parse())
  gg_now = gg_now + theme_grey( base_size = gg_ptsz-5
                              , base_family = "Helvetica"
                              , base_line_size = 0.5
                              , base_rect_size = 0.5)
  gg_now = gg_now +  theme( axis.text.x = element_text( size   = gg_ptsz-5
                                                      , margin = unit(rep(0.35,times=4),"cm")
                                                      )#end element_text
...
gg_patch = wrap_plots(decomp_plot)
gg_patch = gg_patch + guide_area() + plot_layout( guides="collect",ncol=4)
gg_patch = gg_patch + plot_annotation(title = "")
for (d in sequence(ndevice)){
    f_output = paste0("variance-decomp-allcase.",gg_device[d])
    dummy    = ggsave( filename = f_output
                     , plot     = gg_patch
                     , device   = gg_device[d]
                     , path     = plot_main
...
var                  = rownames(sensall_bf)
sensall_madf         = as_tibble(sensall_bf)
sensall_madf$model_var  = var
sensall_madf$type       = "main"


sensall_decom           = sensall_madf                                        %>%  
                          pivot_longer(cols =case:o5_fates_frag_maxdecomp
                                      ,names_to  = "parameter"
                                      ,values_to = "var_decomp")   

par_labelx1            = c("BaseParameters","FBD[dead]","FBD[live]","Ignition","DryingRatio","FuelEnergy","MaxDecomp")
par_labelx2            = c("BaseParameters","GPP","AGB","LAI","Mort[Cstar]","Mort[Hydro]","Mort[Term]",
                           "Mort[Can]","Mort[Ustor]",#"Burnt[A]","Fire[FDI]","Fire[ROS]","Fire[FBD]","Param[FBD-dead]",
                          #"Param[FBD-live]","Param[Nigni]","Param[Drying]","Param[Energy]",
                          "Param[Max-decomp]")

par_label = par_labelx1

sensall_decom        = sensall_decom                                                %>% 
                       mutate( var_decomp = round(as.numeric(var_decomp)*100,1)
                             , label      = par_label) 
  
loop_list   = which(hlm1dvar$vnam %in% decomall_y1)
decomp_plot=list()
for(d in loop_list){
   ...
   h_legend = v == 1
   cat0("   - ",h_desc,".")
   decomp_now = sensall_decom %>% filter(model_var==h_vnam)
   decomp_now = decomp_now %>% arrange(var_decomp)
   decomp_now = decomp_now %>% mutate(label=factor(label,levels=unique(label))) 
  ...
  gg_now = gg_now + theme_bw( base_size = gg_ptsz*1.5
                              ...
                              , base_line_size = 0.5
                              , base_rect_size = 0.5)
  gg_now = gg_now +  theme( axis.text.x = element_text( size   = gg_ptsz-2
                                                      , margin = unit(rep(0.35,times=4),"cm")
                                                      )#end element_text
                          , axis.text.y = element_text( size   = gg_ptsz-2
                                                      , margin = unit(rep(0.35,times=4),"cm")
                                                      )#end element_text
                          , axis.ticks.length = unit(-0.25,"cm")
                          , legend.position   = "none"
  ...
  for (d in sequence(ndevice)){
    f_output = paste0(type,"-variance-decomp1.",gg_device[d])
    dummy    = ggsave( filename = f_output
                     , plot     = gg_now
                     , device   = gg_device[d]
                     , path     = plot_main
...
decomp_plot_reorder = list(gg_bf,gg_fl)
...
## we first filter ensemble members using site obervations
ens_filt    = mmean_com       %>% 
              filter(case!=0) %>% 
...
## next we filter ensembles using mean burned fraction from 
## nearest site 

fire_site    = fread(frap_site)

bfrac_dmean  = fire_site                                      %>% 
               group_by(year)                                 %>% 
               summarize(bfrac_mean=mean(bfrac,na.rm=TRUE))   %>% 
               mutate(bfrac_mean = bfrac_mean/yr.day)         %>% 
               ungroup()


bfrac_qupr   = fire_site                                      %>% 
               group_by(year)                                 %>% 
               summarize(bfrac_qupr=quantile(bfrac
                                            ,probs=qupr_ribbon
                                            ,na.rm=TRUE))     %>% 
               mutate(bfrac_qupr = bfrac_qupr/yr.day)         %>% 
               ungroup()
               
bfrac_qlwr   = fire_site                                      %>% 
               group_by(year)                                 %>% 
               summarize(bfrac_qlwr=quantile(bfrac
                                            ,probs=qlwr_ribbon
                                            ,na.rm=TRUE))     %>% 
               mutate(bfrac_qlwr = bfrac_qlwr/yr.day)         %>% 
               ungroup()

filt_fire    = hlm1d                                             %>% 
               dplyr::select(case,ens_label,time,fates_burnfrac) %>% 
               mutate(case_ens = paste0(case,"_",ens_label))     %>% 
...
filt_fire    = filt_fire                                         %>% 
               ...
               summarize(bfrac_model = 
                           mean(fates_burnfrac,na.rm=TRUE),
                         bfrac_modmedian = median(fates_burnfrac,na.rm=TRUE))      %>% 
               ungroup()

filt_fire    = filt_fire                        %>% 
               left_join(bfrac_dmean,by="year") %>% 
               left_join(bfrac_qupr,by="year")  %>% 
               left_join(bfrac_qlwr,by="year")
filt_fire    = filt_fire                        %>% 
               mutate(year_kp = if_else(
                 bfrac_model >= bfrac_qlwr &
                 bfrac_model <= bfrac_qupr,"keep","drop"
               ))
filt_fire    = filt_fire                             %>% 
               group_by(case,ens_label,case_ens)     %>% 
               mutate(kp_cnt = sum(year_kp=="keep"
                                   ,na.rm=TRUE))     %>% 
               filter(kp_cnt >=6)                    %>% 
               ungroup()
  
keep_site = unique(filt_fire$case_ens) #there are 4 ensembles 

## use regional burned fraction range 
cags_bf    = fread(frap_grass_wrf)       
cags_bf    = cags_bf                       %>% 
...
cags_dmean = cags_bf                                   %>%
             group_by(lon,lat)                         %>% 
             summarize(bfrac=mean(bfrac,na.rm=TRUE))   %>% 
             ungroup() 
cagsmean_upr = quantile(cags_dmean$bfrac,probs=qupr_ribbon,na.rm=TRUE)
cagsmean_lwr = quantile(cags_dmean$bfrac,probs=qlwr_ribbon,na.rm=TRUE)
cagsmean_median = median(cags_dmean$bfrac,na.rm=TRUE)

## range of annual burned fraction by each year using regional data         
cags_quantyr = cags_bf                                 %>% 
               group_by(year)                          %>%
...
filt_fire    = hlm1d                                             %>% 
               dplyr::select(case,ens_label,time,fates_burnfrac) %>% 
               mutate(case_ens = paste0(case,"_",ens_label),
                      fates_butnfrac = fates_burnfrac*yr.day)    %>% 
               filter(case_ens %in% ens_keep)
filt_fire    = filt_fire                                         %>% 
               mutate(month=month(time))                         %>% 
               filter(month %in% fire_month)                     %>% 
               ...
               summarize(bfrac_model = 
                           mean(fates_burnfrac,na.rm=TRUE),
                         bfrac_modmdian = median(fates_burnfrac,na.rm=TRUE))      %>% 
               ungroup()

filt_fire$mean_qlwr = cagsmean_lwr
filt_fire$mean_qupr = cagsmean_upr
filt_fire$mean_median = cagsmean_median


## filter with low and max mean annual bf

filt_fire     = filt_fire                                         %>% 
                filter(bfrac_model >=mean_qlwr & bfrac_model<=mean_qupr)  

filt_fire     = filt_fire                                         %>% 
                mutate(rela_bf = bfrac_modmdian/cagsmean_median)     %>% 
                filter(rela_bf >= 0.99 & rela_bf <= 1.01)
#keep_region1 = filt_fire$case_ens[order(filt_fire$bfrac_model,decreasing=TRUE)[2000]]
keep_region1 = unique(filt_fire$case_ens)

## too many 


## approach2: filter by annual trend
filt_fire    = hlm1d                                             %>% 
               dplyr::select(case,ens_label,time,fates_burnfrac) %>% 
               mutate(case_ens = paste0(case,"_",ens_label),
                      fates_burnfrac = fates_burnfrac*yr.day)    %>% 
               filter(case_ens %in% ens_keep)
filt_fire    = filt_fire                                         %>% 
               mutate(month=month(time),year=year(time))         %>% 
               filter(month %in% fire_month)                     %>% 
               group_by(case,ens_label,case_ens,year)            %>%
               summarize(fates_burnfrac = 
                           mean(fates_burnfrac,na.rm=TRUE))      %>% 
               ungroup()                       
...
filt_fire    = filt_fire                                         %>% 
               ...
               group_by(case,ens_label,case_ens)                 %>% 
               mutate(mark = if_else(fates_burnfrac>=yr_qlwr & 
                                     fates_burnfrac<=yr_qupr
                                     ,"keep"
                                     ,"drop"))                   %>% 
               mutate(kep_cnt = sum(mark=="keep"))               %>% 
               ungroup()
filt_fire    = filt_fire                                         %>% 
               filter(kep_cnt >=11)
                                    
keep_region2 = unique(filt_fire$case_ens) #6 ensemble members

## approach 3: relative annual burned fraction is close to 1
...
filt_fire    = filt_fire                                         %>% 
               left_join(cags_quantyr,by="year")                 %>% 
               group_by(case,ens_label,case_ens)                 %>% 
               mutate(rela_bf = fates_burnfrac/yr_mean)          %>% 
               mutate(mark = if_else(rela_bf>=0.5 & 
                                     rela_bf<=1.5
                                     ,"keep"
                                     ,"drop"))                   %>% 
               mutate(kep_cnt = sum(mark=="keep"))               %>% 
...
keep_region3 = unique(filt_fire$case_ens)

keep_final  = c(keep_site,keep_region1,keep_region2,keep_region3)
keep_final  = unique(keep_final)
...
```{r,label="kept-members-annual-fire-dynamic",message=FALSE,results='hide'}

fireamean_keep =  hlm1d                                             %>% 
                  dplyr::select(case,ens_label,time,fates_burnfrac) %>% 
                  mutate(case_ens = paste0(case,"_",ens_label))     %>% 
                  filter(case_ens %in% keep_region2)
fireamean_keep = fireamean_keep                                    %>% 
                 mutate(month=month(time),year=year(time))         %>% 
                 filter(month %in% fire_month)                     %>% 
                 group_by(case,ens_label,case_ens,year)            %>%
                 summarize(fates_burnfrac = 
...
for(h in fire_loop){
  ...
  fire_now        = fireamean_keep
   #mmean_now$case = factor(mmean_now$case,levels=unique(mmean_now$case))
   
   gg_now = ggplot(data=fire_now)
   gg_now = gg_now + facet_wrap(.~case,labeller=labeller(case=case_desc))
   gg_now = gg_now + geom_line(aes_string(x="year",y=h_vnam,group="case_ens",color="case_ens"),lwd=0.7,alpha=0.8,show.legend=TRUE)
   gg_now = gg_now + geom_line(aes(x=year,y=yr_mean,color="FRAP"))
   gg_now = gg_now + scale_color_manual(name="",values=c(top12[1:6],site_colour))
   #gg_now = gg_now + scale_colour_manual(name=""
   #                                     ,values=c("Model"= ens_colour[1]
   #                                              ,"FRAP" = site_colour))
