...
local_path  = file.path("~/Google Drive/My Drive/all/regional-runs/regional-runs-set3")
main_path   = file.path("~/Google Drive/My Drive/all")
region_path = file.path(main_path,"regional-runs")
#nc_path     = file.path(region_path,"regional-runsp1")
util_path   = file.path(home_path,"Util/RUtils")

## set 2
#cases       = c( "regional-avba-N08T27_CLM_FATES.clm2.nc"
#               , "regional-quant-genl-N12T25_CLM_FATES.clm2.nc"
#               , "regional-site-genl-N02T19_CLM_FATES.clm2.nc"
#               , "regional-site-genl-N04T06_CLM_FATES.clm2.nc"
#              )
...
case_files   = file.path(local_path, cases)
ncase        = length(cases)
case_names   = c(1L,2L,3L)

case_desc    = c( "Low-GENL-G1-N05T08"
               ,  "Medium-GENL-G1-N08T27"
               ,  "High-GENL-G1-N12T25"
               )

names(case_desc) = c("1","2","3")
...
tbf_grass   = file.path(frap_main,"total-annual-BF-grasslands-3sources.csv")
...
gpp_ob       = gpp_ob %>% mutate(lon = round(lon,digits=5),
                                 lat = round(lat,digits=5))
...
lai_ob = lai_ob %>% mutate(lon = round(lon,digits=5),
                           lat = round(lat,digits=5))
...
## load simulation using BRDI allometry
hlm1d_2 = fread("~/Google Drive/My Drive/all/regional-runs/Figures-set1/region_hlm1d.csv")
hlm1d_2 = hlm1d_2 %>% filter(case==2) 
hlm1d_2 = hlm1d_2 %>% mutate(case=4)
hlm1d   = rbind(hlm1d, hlm1d_2)
case_desc2 = c("Mean-BRDI-G3-N09T04")
names(case_desc2)=c("4")
case1d_desc = c(case_desc, case_desc2)
ncase_1d  = length(case_desc)
case1d_names = c(1L,2L,3L,4L)
...
gg_emean   = list()
for(c in sequence(ncase_1d)){
  case_now = case1d_names[c]
  desc_now = case1d_desc[c]
...
### first calculate RMSE for predicted GPP, LAI, and BA

rmse_fun  = function(mod, obs){
  rmse = sqrt(mean((mod-obs)^2,na.rm=TRUE))
  return(rmse)
}

rmse_df   = grosea_hlmcom                                        %>% 
            filter(!is.na(fates_gpp))                            %>% 
            group_by(case)                                       %>% 
            summarize(gpp_rmse = rmse_fun(mod = fates_gpp,
                                          obs = gpp),
                      gpp_rng  = max(gpp,na.rm=T) - min(gpp,na.rm=T),
                      lai_rmse = rmse_fun(mod = elai,
                                          obs = lai),
                      lai_rng = max(lai)-min(lai),
                      ba_rmse  = rmse_fun(mod = fates_burnfrac,
                                          obs = bfrac),
                      ba_rng  = max(bfrac) - min(bfrac))
rmse_df = rmse_df %>% mutate(gpp_nrmse = gpp_rmse/gpp_rng,
                             lai_nrmse = lai_rmse/lai_rng,
                             ba_nrmse  = ba_rmse/ba_rng)
...
for(n in sequence(ncase_1d)){
  ...
  #scale_fill_continuous(low="blue", high="red", na.value="white", guide="colorbar", limits=ylim,oob = scales::squish)+
    ...
    scale_x_continuous(limits = c(-124, -113)) +
    scale_y_continuous(limits = c(32, 42.5)) + 
    labs(x=NULL,y=NULL,title="CLM-FATES") +
    ...
    theme(  legend.position  = "none"
        , legend.title     = element_text(size=gg_ptsz)
        , legend.text      = element_text(size=gg_ptsz)
        , plot.title       = element_text(size=gg_ptsz, vjust=1)
        , panel.background = element_blank()
        #, panel.border     = element_rect(linewidth = 1.6, fill=NA)
...
#      ) #end theme
 if(l==3){
   ...
   scat = scat + labs(x="Model",y="Observation")
   if(n==3){scat = scat + scale_x_continuous(limits = c(0,0.01),n.breaks= 3)}else{
    scat = scat + scale_x_continuous(n.breaks= 3) 
   }
   scat = scat + geom_abline(slope=1,intercept=0,linetype="dashed",color="black",linewidth=1)
   scat = scat + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
   scat = scat + 
         theme( legend.position  = "none" 
              , panel.background = element_blank()
              , panel.border     = element_rect(linewidth = 1.6, fill=NA)
  ...
  p1 = mod +  plot_spacer() + obs  +
     plot_layout(guides="keep",widths = c(1.5,-0.4,1.5,1), nrow=1) 
  p2 = p1 + scat
  #p = wrap_plots(mod,obs,scat) + 
     #plot_layout(guides="collect",width=c(2,2,1),ncol=3) 
   all_plots[[l]] = p2
   fname = paste0(case_now,"-",var_now,"_model-obs.png")
   ggsave(p2, filename=fname,path=secy_path,width=gg_widthn*0.6,height=gg_height*0.45,dpi=gg_depth,device=gg_device[1]) 
  ...
  gg_patch = all_plots[[1]] / all_plots[[2]] / all_plots[3]
  for (d in sequence(ndevice)){ 
