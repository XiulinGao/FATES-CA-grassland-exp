...
ens_cases   = c("avba-fireoff-280.nc","avba-fireoff-400.nc","brdi-fireoff-280.nc",
                "brdi-fireoff-400.nc","genl-fireoff-280.nc","genl-fireoff-400.nc")
now_case    = ens_cases[4]
case_name   = substr(now_case,1,16)
case_file   = file.path(main_path,now_case)
...
#plot path
plot_main  = file.path(main_path,"Figures",case_name)
...
nc_conn  = nc_open(filename=case_file)
...
for(a in sequence(nbyage)){
  ...
  byage[[f_vnam]] = matrix(data=NA_real_,nrow=ntstamp_each*ninst,ncol=nages,dimnames=list(NULL,ageinfo$key)) 
} #end of a loop
...
for(d in sequence(nbydbh)){
  ...
  f               = match(nc_pref,fatesvar$vnam)
  f_vnam          = fatesvar$vnam[f]
  bydbh[[f_vnam]] = matrix(data=NA_real_,nrow=ntstamp_each*ninst,ncol=ndbhs,dimnames=list(NULL,dbhinfo$key))
} # end of d

# by pft

#pfvar = c("fates_mortality_cstarv_cflux","fates_mortality_fire_cflux",
          #"fates_mortality_hydraulic_cflux","fates_recruitement")
bypft = list()
for(p in sequence(nbypft)){
  ...
  f               = match(nc_pref,fatesvar$vnam)
  f_vnam          = fatesvar$vnam[f]
  bypft[[f_vnam]] = matrix(data=NA_real_,nrow =ntstamp_each*ninst,ncol=npfts,
...
hlm1d = as_tibble( matrix( data     = NA_real_
                         , nrow     = ntstamp_inst
                         , ncol     = nhlm1d+2
                         , dimnames = list(NULL,tolower(c(nc_hlm1d, "ENS_LABEL","CASE")))
                         )
                 )
...
for(m in sequence(nhlm2dsoi)){
  ...
  hlm2d[[h_vnam]] = matrix(data=NA_real_,nrow=ntstamp_inst,ncol=nsois,dimnames=list(NULL,soikeys))
}
...
#for(n in sequence(ntstamp_each)){
 ...
 # w_year       = year(each_tstamp[w])
  ...
  # we skip unit conversion for variables where f_mult is 
  # cmon.sec as we cannot find cmon.sec w/o reading time slice one by one
  # so we change f_mult for those variables in hlm_varlist.r to 1.
  # and use mutate() to do unit conversion once we have the entire 
  # dataset with time column appended. 
  # these variables include: qdirect_throughfall,qdrai,qdrip,qevtr
  # qintr,qover,qsoil,qvege,qvegt,rain,the goal is to convert those to mm/month
  # by multiplying seconds/month
  
   cmon.day = days_in_month(each_tstamp[w])
   cmon.hr  = day.hr  * cmon.day
   cmon.min = day.min * cmon.day
   cmon.sec = day.sec * cmon.day
last_hlm1d  = hlm1dvar$vnam[hlm1dvar$mult=="cmon.sec"] 
...
#      nc_dat              = f_add0 + f_mult * ncm_dat
#      bydbh[[f_vnam]][,]  = nc_dat #here we skip aggregating data for size class for by SZPF variable as we                                       #only have 1 PFT, so even SZPF is also a true size class variable
...
#      v_vcan = paste0(v_vnam,"_canopy")
      ...
      # Aggregate data
#      bydbh[[v_vnam]] = bydbh[[v_vund]] + bydbh[[v_vcan]]
...
for (p in sequence(nbypft-n_varpft_last)){
   ...
   for (v in which(varpft_last)){
      ...
      # Aggregate data
      bypft[[v_vnam]] = bypft[[v_vund]] + bypft[[v_vcan]]
 ...
 # Read drought-deciduous phenology variables (by PFT), and assign current values to the matrix.
   ...
   for (p in sequence(ndphen)){
      ...
      nc_dat              = ncvar_get(nc=nc_conn,varid=nc_nvnow)
      dphen[[f_vnam]][,] = f_add0 + f_mult * rev(nc_dat)
   }#end for (a in sequence(nbyage))
...
for (v in sequence(nhlm1d-etr_last-er_last)){
  ...
  if(h_vnam %in% last_hlm1d){h_mult=1.}else{h_mult=h_mult} #update h_mult to 1 to skip unit conversion for monthly variables
  
  hlm1d[[h_vnam]]       = h_add0 + h_mult * ncm_dat
  hlm1d[['ens_label']]  = rep(ens_labels,each = ntstamp_each)
  hlm1d[['case']]       = rep(case_name,time=ntstamp_each*ninst)
}#for (h in sequence(nhlm1d-etr_last-et_last))

# Find total ET.
if (etr_last){
  
hlm1d$qevtr = hlm1d$qvege + hlm1d$qvegt + hlm1d$qsoil
}#end if (etr_last)

# Find total ET.
if (er_last){
      
hlm1d$er = hlm1d$fates_autoresp + hlm1d$fates_het_resp
...
# Close connection
dummy   = nc_close(nc_conn)
#}#end of n in sequence(ntstamp_each)
...
#--- Match variables
  ...
  if (f_stack && (! (f_vnam %in% "fates_patcharea"))){
      cat0("   - ",f_desc,".")
      byage[[f_vnam]] = byage[[f_vnam]] * byage$fates_patcharea
...
#--- Create molten data table for this variable.  
  ...
  now_age$case      = rep_along(time, x = case_name)
  ...
  if (is.null(age_melt)){
      age_melt = now_melt
        ...
        age_melt = as_tibble(merge(x=age_melt,y=now_melt,by=c("case","ens_label","time","age"),all=TRUE))
...
#  now_dbh$case      = rep_along(time, x = case_name)
...
for (p in sequence(nbypft)){
  ...
  now_pft$case      = rep_along(time, x=case_name)
  ...
  if (is.null(pft_melt)){
      pft_melt = now_melt
         ...
         pft_melt = as_tibble(merge(x=pft_melt,y=now_melt,by=c("case", "ens_label", "time", "pft"),all=TRUE))
      }
...
if (! is_tibble(dphen)){
   ...
   for (p in sequence(ndphen)){
      ...
      now_dph$time = tstamp
      now_melt     = as_tibble(melt(data=now_dph,id.vars=c("case","ens_label","time"),variable.name="pft",value.name=f_vnam))

      # Merge data table
      if (is.null(dph_melt)){
         dph_melt = now_melt
      }else{
         dph_melt = as_tibble(merge(x=dph_melt,y=now_melt,by=c("case","ens_label","time","pft"),all=TRUE))
...
for (s in sequence(nhlm2d)){
  h        = match(names(hlm2d)[s],hlm2dsoi$vnam)
  h_vnam   = hlm2dsoi$vnam[h]
...
```{r,label="unit-conversion-for-hlm1dvar",message=FALSE,results='hide'}
...
```{r,label="param-hist-plot",message=FALSE}

Hmisc::hist.data.frame(param_survi[c(1:2,4:21)])
Hmisc::hist.data.frame(param_tbl[c(1:2,4:21)])
Hmisc::hist.data.frame(param_dead[c(1:2,4:21)])
...
```{r,label="annual mean of each variable"}

cat0("Plot anual mean distributions of each variable for survived ebsembles")
Hmisc::hist.data.frame(survi_amean[4:15])
...
ob_cue  = site1d %>% mutate(month = month(time)) %>% 
...
ggplot(plot_df, aes(fates_gpp,fates_npp)) +
  ...
  geom_point(data=ob_cue,aes(gpp,npp,colour="Site"))   +
  scale_colour_manual(values=c("Site"="blue")) +
  theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,
              base_rect_size =0.5) +
              theme(legend.position    = "right"
           , legend.text=element_text(size=gg_ptsz)
...
top_10labels = unique(top_ten$ens_label)


top10_mmean  = mmean_com                                                      %>% 
               filter(ens_label %in% c(top_10labels,"site"))                  %>% 
               arrange(source,ens_label)                                      

leg_labels  = unique(top10_mmean$ens_label)

top10_mmean = top10_mmean                                                      %>% 
...
for (h in emean_loop){
  ...
  #mmean_now$source = factor(mmean_now$source,levels=unique(mmean_now$source))
   ...
   gg_now = ggplot(data=mmean_now,aes_string(x="month",y=h_vnam,group="ens_label",colour="ens_label"))
   gg_now = gg_now + scale_colour_manual(name="",aesthetics="colour",labels=leg_labels,values=leg_colours)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(h_vnam, case_name,"top10.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot     = gg_now
                    , device   = gg_device[d]
                    , path     = secy_path
...
```{r,label="parameter-distribution-top10"}

top10_par = param_survi                          %>% 
            filter(ens_label %in% top_10labels)
...
cor_survi   = survi_all[c(7,11:15, 17:21,23:33,36:39)] 

cor_allens   = all_mmean                            %>% 
               filter(month==4)                     %>% 
               left_join(param_tbl,by="ens_label")  
cor_allens  = cor_allens[c(7,11:15, 17:21,23:33,36:39)]
...
corr_surviens     = cor(cor_survi)
corr_allens       =cor(cor_allens)
corrplot::corrplot(corr_surviens
                ,method = "color"
                ,addCoef.col="grey"
                ,order = "AOE"
                ,number.cex=1.3
...
if (plot_top10){
   ...
   gg_mten = gg_mten + facet_wrap(.~ ens_label, ncol = 3, labeller = label_parsed)
   ...
   gg_mten = gg_mten + labs(title=case_name)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
      m_output = paste0("mort-top10-",case_name,".",gg_device[d])
...
asses_vars   = names(survi_mmean)[c(7,11:15,17:18)]
...
sens_survi   = sens_survi                             %>% 
               left_join(param_survi,by="ens_label")
sens_all     = sens_all                               %>% 
               left_join(param_all,by="ens_label")

sens_main = matrix( data=NA_real_
                  ,nrow=n_var
                  ,ncol=n_param
                  ,dimnames = list(asses_vars,pars))
sens_plot = list()
for(v in sequence(n_var)){
  var_now = asses_vars[v]
  for(p in sequence(n_param)){
    p_now = pars[p] 
    #df_now = sens_survi %>% select(p_now,var_now)
    df_now = sens_all %>% dplyr::select(p_now,var_now)
    ...
    for (d in sequence(ndevice)){
    f_output = paste0(var_now,"-",p_now,"-varmain.",gg_device[d])
...
for(v in sequence(n_var)){
 ...
 # y       = sens_survi[[var_now]]
  ...
  for(p in sequence(n_param)){
    ...
    for (d in sequence(ndevice)){
    f_output = paste0(var_now,"-",p_now,"-varmin.",gg_device[d])
    dummy = ggsave( filename = f_output
                  , plot     = gg_now
                  , device   = gg_device[d]
                  , path     = sens_path
...
sens_main = as_tibble(sens_main)
sens_main$model_var = var
sens_main$type       = "main"

var       = rownames(sens_min)
sens_min  = as_tibble(sens_min)
sens_min$model_var = var 
sens_min$type       = "min"
sens_decom            = rbind(sens_main,sens_min)
sens_decom            = sens_decom %>%  
                        pivot_longer(cols = turb_diameter:frag_maxdecomp,
                                  names_to = "parameter",
                                  values_to = "var_decomp")

ggplot(sens_decom,aes(var_decomp,parameter)) +
  geom_point(aes(x=var_decomp,y=parameter,color=type,group=type),size=2.5) +
  geom_linerange(aes(xmin=0,xmax=var_decomp,color=type),
                 size=3, alpha=0.4)+#,position = position_dodge(width = 0.2)) +
  facet_wrap(.~model_var) + 
  theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size = 0.5,base_rect_size =0.5) +
  theme( axis.text.x       = element_text( size   = gg_ptsz
                                                              , margin = unit(rep(0.35,times=4),"cm")
                                                              )#end element_text
...
#ggsave(filename="variance-decomp-survi.png",path=plot_main,width=gg_widthn,height=gg_height*1.2,unit="in",dpi=gg_depth)
ggsave(filename="variance-decomp-all.png",path=plot_main,width=gg_widthn,height=gg_height*1.2,unit="in",dpi=gg_depth)
