...
flux_base   = list.files(path=site_main,pattern="_eddy-summ\\.nc$"  )
agb_base    = list.files(path=site_main,pattern="biomass")
lai_base    = list.files(path=site_main,pattern="LAI\\.csv$")
seablm_path = file.path(seablm_main,"nutnet-npp-div-data-output.csv")
flux_path   = file.path(site_main,flux_base)
agb_path    = file.path(site_main,agb_base)
...
gg_device  = c("tiff")     # Output devices to use (Check ggsave for acceptable formats)
...
#--- Create molten data table for this variable.  
  ...
  if (is.null(now_agemelt)){
    now_agemelt = now_melt
        ...
        now_agemelt = now_agemelt %>% left_join(now_melt,by=c("case","ens_label","time","age"))
...
#fwrite(byage,file.path(main_path,paste0("allcase_byage-WTCrownarea.csv")))
...
bypft = fread(file.path(main_path,"allcase_bypft.csv"))
hlm1d = fread(file.path(main_path,"allcase_hlm1d.csv"))
byage = fread(file.path(main_path,"allcase_byage.csv"))
...
p_vlist       = param_config$fates_parameter[!grepl(pattern="fire",param_config$fates_parameter)] 
np            = length(p_vlist)
...
for (p in sequence(np)){
  ...
  nc_nvnow            = p_vlist[p]
  h                   = match(nc_nvnow,param_config$fates_parameter)
...
all_mmean = all_ens                                                    %>% 
 ...
 survi_mean = survi_ens                                                %>% 
              group_by(case,ens_label,case_ens,pft)                    %>% 
  summarize( fates_ddbh_canopy            = mean(fates_ddbh_canopy)
           , fates_ddbh_ustory            = mean(fates_ddbh_ustory)
           , fates_gpp                    = mean(fates_gpp)
           , fates_lai_canopy             = mean(fates_lai_canopy)
...
cue_mod = survi_ens %>% dplyr::select(fates_gpp,fates_npp,case,ens_label,case_ens,month) %>% 
  mutate(cue = cue_fun(x=.data$fates_npp,n=.data$fates_gpp))

cue_mod = cue_mod %>% group_by(case,ens_label,case_ens,month) %>% 
...
for (h in emean_loop){
   ...
   df_now           = mmean_com %>% filter(case %in% c(4,5,6))
   #df_now           = subset(df_now,case!=0)
   obs              = site_mmean 
   #mmean_now$case = factor(mmean_now$case,levels=unique(mmean_now$case))
   
   gg_now = ggplot(data=df_now)
   gg_now = gg_now + facet_wrap(.~case,ncol = 1, labeller=labeller(case=case_desc))
...
```{r,label="plot-allometry-effecs",message=FALSE}

plot_allom = hlm1d                                          %>% 
             filter(case %in% c(4,5,6))                     %>%
             mutate(case_ens = paste0(case,"_",ens_label))  %>% 
              
             dplyr::select(case,ens_label,case_ens,time
                          ,fates_gpp,fates_vegc_aboveground
                          ,elai,fsh,eflx_lh_tot)            %>% 
             mutate(alom_grp = case_when(case %in% c("4") ~ "AVBA"
                                        ,case %in% c("5") ~ "BRDI"
                                        ,case %in% c("6") ~ "GENL")
                    )                                        %>% 
             mutate(month = month(time))                     %>% 
             #filter(month %in% growing_sea)                  %>% 
             group_by(case,ens_label,case_ens,alom_grp)      %>% 
...
allom_survi  = plot_allom                       %>% 
               filter(case_ens %in% survi_label)

n_plot       = length(hlm_var)

#alom_loop = which(hlm1dvar$vnam %in% hlm_var)

## HSD test
gpp_hsd = HSD.test(aov(fates_gpp~alom_grp,data=plot_allom),"alom_grp",group=TRUE)
lai_hsd = HSD.test(aov(elai~alom_grp,data=plot_allom),"alom_grp",group=TRUE)
agb_hsd = HSD.test(aov(fates_vegc_aboveground~alom_grp,data=plot_allom),"alom_grp",group=TRUE)
lh_hsd  = HSD.test(aov(eflx_lh_tot~alom_grp,data=plot_allom),"alom_grp",group=TRUE)
sh_hsd  = HSD.test(aov(fsh~alom_grp,data=plot_allom),"alom_grp",group=TRUE)

## have to mannualy assign letters given HSD test as the order is not consistent
gpp_label = c("a","b","b")
lai_label = c("a","ab","b")
agb_label = c("a","b","c")
lh_label  = c("a","a","a")
...
## set location of text to be on top 
gpp_lo  = plot_allom %>% group_by(alom_grp) %>% summarize(lo = max(fates_gpp) + 0.3)
lai_lo  = plot_allom %>% group_by(alom_grp) %>% summarize(lo = max(elai) + 0.3 )
agb_lo  = plot_allom %>% group_by(alom_grp) %>% summarize(lo = max(fates_vegc_aboveground) + 0.3)
lh_lo   = plot_allom %>% group_by(alom_grp) %>% summarize(lo = max(eflx_lh_tot) + 1)
sh_lo   = plot_allom %>% group_by(alom_grp) %>% summarize(lo = max(fsh) + 1)

## tweak text location for agb plot
agb_lo$lo <- c(0.2, 0.29, 0.22)
agb_lo$label = agb_label
gpp_lo$label = gpp_label
lai_lo$label = lai_label
lh_lo$label = lh_label
sh_lo$label = sh_label



txt_lo   = c("gpp_lo","agb_lo","lh_lo","lai_lo","sh_lo")
...
for (n in sequence(n_plot)){
   ...
   alom_now        = plot_allom
   ymax           = max(plot_allom[[var_now]])*1.15
   ymin           = min(plot_allom[[var_now]])

   gg_now = ggplot(data=alom_now)
   gg_now = gg_now + geom_boxplot(aes_string(x="alom_grp",y=h_vnam,colour="alom_grp")
                              ,show.legend=TRUE)
   gg_now = gg_now + coord_cartesian(ylim = c(ymin, ymax))
   gg_now = gg_now + geom_text(data=lo_now, aes(x=alom_grp,y=lo,label=label),vjust=0, size=6) 
  
   #gg_now = gg_now + stat_compare_means(aes_string(x="alom_grp",y=h_vnam),method="t.test",ref.group = "AVBA",
   #                                      label = "p.signif",hide.ns=TRUE,label.y.npc=0.97,size=5.3)
   
   gg_now = gg_now + scale_colour_manual(name="",values=ens_colour)
   #gg_now = gg_now + labs(title=h_desc)
   gg_now = gg_now + labs(title="")
   gg_now = gg_now + xlab(element_blank())
   gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
   gg_now = gg_now + theme_bw( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
  ...
  gg_now = gg_now + guides(colour = guide_legend(ncol=1))
   
  gg_alom[[h_vnam]] = gg_now      

}

gg_alom_reorder  = list(gg_alom$fates_gpp,gg_alom$elai,gg_alom$fates_vegc_aboveground,gg_alom$eflx_lh_tot,gg_alom$fsh)
gg_patch = wrap_plots(gg_alom_reorder)
...
# Save plots.
for (d in sequence(ndevice)){
    h_output = paste0("allomeffects_allpeak.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_patch
                    , device    = gg_device[d]
                    , path      = secy_path
...
## canopy cover for each allometry group
byage = byage %>% dplyr::select(case,ens_label,time,age,fates_canopyarea) %>% filter(case %in% c(4,5,6))
ca_sum = byage %>% group_by(case,ens_label,time) %>% summarize(fates_canopyarea = sum(fates_canopyarea))

ca_sum = ca_sum                     %>%   
         ungroup()                  %>% 
         group_by(case,ens_label)   %>%
         summarize(fates_canopyarea = mean(fates_canopyarea)) %>% 
         mutate(alom_grp = case_when(case %in% c("4") ~ "AVBA"
                                    ,case %in% c("5") ~ "BRDI"
                                    ,case %in% c("6") ~ "GENL")) %>% 
         ungroup()
...
gg_now = ggplot(data=ca_sum)
gg_now = gg_now + geom_boxplot(aes(x=alom_grp,y=fates_canopyarea,colour=alom_grp)
                              ,show.legend=TRUE)
gg_now = gg_now + scale_colour_manual(name="",values=ens_colour)
gg_now = gg_now + labs(title="")
gg_now = gg_now + xlab(element_blank())
gg_now = gg_now + ylab(expression("Crown area"~"("~m^2~m-2~")"))
...
for (d in sequence(ndevice)){
    h_output = paste0("allomeffects_canopycover.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_now
                    , device    = gg_device[d]
                    , path      = secy_path
...
```{r,label="plot-difference-between-co2-treatments"}

survi_co2 = as_tibble(survi_label)
survi_co2 = survi_co2                         %>% 
            mutate(case = substr(value,1,1)
                  ,ens  = substr(value,3,20)) %>% 
            filter(case %in% c("4","5","6"))  %>% 
            mutate(cores_case = case_when(case == "4" ~ "1"
                                         ,case == "5" ~ "2"
                                         ,case == "6" ~ "3")
                  ,cores_survi= paste0(cores_case,"_",ens))

survi_280 = survi_co2$cores_survi
survi_400 = survi_co2$value


delta_co2 = hlm1d                                          %>% 
            mutate(case_ens = paste0(case,"_",ens_label))  %>% 
            #filter(case_ens %in% c(survi_280,survi_400))   %>% 
            dplyr::select(case,ens_label,case_ens,time
                         ,fates_gpp,fates_vegc_aboveground
                         ,elai,fsh,eflx_lh_tot)            %>% 
            mutate(alom_grp = case_when(case %in% c("1","4") ~ "AVBA"
                                       ,case %in% c("2","5") ~ "BRDI"
                                       ,case %in% c("3","6") ~ "GENL")
                  ,co2_trt  = if_else(case %in% c("1","2","3")
                                     ,280,400))            %>% 
            dplyr::select(-c(case,case_ens))               %>% 
            pivot_wider(names_from  = co2_trt
                       ,values_from = c(fates_gpp,fates_vegc_aboveground
                                       ,elai,fsh,eflx_lh_tot))

deltaR_co2 = delta_co2                                                          %>% 
            group_by(alom_grp,ens_label,time)                                   %>% 
            mutate(fates_gpp                 = ifelse(fates_gpp_400==0
                                                      ,NA
                                                      ,(fates_gpp_400-fates_gpp_280)/fates_gpp_400)
                     ,elai                   = ifelse(elai_400==0
                                                      ,NA
                                                      ,(elai_400-elai_280)/elai_400)
                     ,fates_vegc_aboveground = ifelse(fates_vegc_aboveground_400==0
                     ,NA
                     ,(fates_vegc_aboveground_400-fates_vegc_aboveground_280)/fates_vegc_aboveground_400) 
                     ,fsh                    = ifelse(fsh_400==0
                                                      ,NA
                                                     ,(fsh_400-fsh_280)/fsh_400)
                    ,eflx_lh_tot            = (ifelse(eflx_lh_tot_400==0
                                                      ,NA
                                                      ,(eflx_lh_tot_400-eflx_lh_tot_280)/eflx_lh_tot_400)))%>%
            ungroup()                            %>% 
            dplyr::select(alom_grp,ens_label,time
                         ,fates_vegc_aboveground
                         ,fates_gpp,elai,fsh
                         ,eflx_lh_tot)

deltaR_mmean = deltaR_co2                        %>% 
              mutate(month=month(time))          %>% 
              dplyr::select(-time)               %>% 
              group_by(alom_grp,ens_label,month) %>% 
              summarize_all(mean,na.rm=TRUE)     %>% 
              ungroup()
...
delta_co2   = delta_co2                                                          %>% 
              group_by(alom_grp,ens_label,time)                                  %>%
              summarize(fates_gpp = fates_gpp_400 - fates_gpp_280
                       ,fates_vegc_aboveground = fates_vegc_aboveground_400 - fates_vegc_aboveground_280
                       ,elai = elai_400 - elai_280
                       ,eflx_lh_tot = eflx_lh_tot_400 - eflx_lh_tot_280
                       ,fsh = fsh_400 - fsh_280) %>% 
              ungroup()

delta_mmean = delta_co2                          %>% 
              mutate(month=month(time))          %>% 
              dplyr::select(-time)               %>% 
              group_by(alom_grp,ens_label,month) %>% 
              summarize_all(mean,na.rm=TRUE)     %>% 
...
delta_loop = which(hlm1dvar$vnam %in% hlm_var)
...
for (h in delta_loop){
   ...
   delta_now        = delta_mmean
   #mmean_now$case = factor(mmean_now$case,levels=unique(mmean_now$case))
   
   gg_now = ggplot(data=delta_now)
   gg_now = gg_now + facet_wrap(.~alom_grp)
   gg_now = gg_now + geom_line(aes_string(x="month",y=h_vnam,group="ens_label",color="'Model'")
                              ,lwd=0.1,alpha=0.8,show.legend=TRUE)
   ...
   gg_now = gg_now + scale_x_continuous( breaks = sequence(12))#end scale_x_continuous
   gg_now = gg_now + xlab(element_blank())
   gg_now = gg_now + ylab(desc.unit(desc=paste0('delta*',h_short),unit=untab[[h_unit]],dxpr=TRUE))
   #gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
   gg_now = gg_now + theme_bw( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
   gg_now = gg_now + theme( legend.position   = "right"
                         , legend.text=element_text(size=gg_ptsz)
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(h_vnam,"_co2DeltaABS.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_now
                    , device    = gg_device[d]
                    , path      = secy_path
  ...
  gg_delta[[h_vnam]] = gg_now      

}


# If sought, plot images on screen
if (gg_screen) gg_delta


##2. BOXPLOT comparing means
...
plot_co2 =   hlm1d                                                   %>% 
             ...
             dplyr::select(case,ens_label,case_ens,time
                          ,fates_gpp,fates_vegc_aboveground
                          ,elai,fsh,eflx_lh_tot)                     %>% 
             mutate(allom_grp= case_when(case %in% c("1","4") ~ "AVBA"
                                        ,case %in% c("2","5") ~ "BRDI"
                                        ,case %in% c("3","6") ~ "GENL")
                  ,co2_grp   = case_when(case %in% c("1","2","3") ~ "280 ppm"
                                        ,case %in% c("4","5","6") ~ "400 ppm")
                    )                                                %>% 
             mutate(month = month(time))                             %>% 
#             filter(month %in% growing_sea)                          %>% 
             group_by(case,ens_label,case_ens,allom_grp,co2_grp)     %>% 
             dplyr::select(-c(time,month))                           %>% 
             summarize_all(mean,na.rm=TRUE)                          %>% 
             ungroup()

co2_loop = which(hlm1dvar$vnam %in% hlm_var)
...
for (h in co2_loop){
   ...
   gg_now = ggboxplot(data=df_now,x="co2_grp",y=h_vnam,facet.by="allom_grp",fill="co2_grp",show.legend=TRUE)
                             
   gg_now = gg_now + stat_compare_means(aes_string(x="co2_grp",y=h_vnam),method="t.test",ref.group = "280 ppm",
                                        label = "p.signif",hide.ns=TRUE,size=4.2,label.y.npc=0.97)
   
   gg_now = gg_now + scale_fill_manual(name="",values=c("white","grey"))
   #gg_now = gg_now + labs(title=h_desc)
   gg_now = gg_now + labs(title="")
   gg_now = gg_now + xlab(element_blank())
   gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
  ...
  gg_co2[[h_vnam]] = gg_now      

}

gg_co2_reorder = list(gg_co2$fates_gpp,gg_co2$elai,gg_co2$fates_vegc_aboveground,gg_co2$eflx_lh_tot,gg_co2$fsh)
gg_patch = wrap_plots(gg_co2_reorder)
gg_patch = gg_patch + guide_area() + plot_layout(guides="collect")
gg_patch = gg_patch + plot_annotation(tag_levels = "a", title = "")
...
# Save plots.
for (d in sequence(ndevice)){
    h_output = paste0("co2ffects_allpeak.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_patch
                    , device    = gg_device[d]
                    , path      = secy_path
...
```{r,label="data-for-variance-decomp-and-correlation-matrix"}

## we use different ways to summarize model outputs to do variance decomposition

#1. the max monthly mean value 
asses_vars   = names(survi_mmean)[c(8,12:16,18:19)]

max_survi   = survi_mmean                                                   %>% 
...
mmean_lh      = hlm1d                                                       %>% 
              dplyr::select(case,ens_label,time,eflx_lh_tot)                %>% 
              mutate(month=month(time))                                     %>% 
              dplyr::select(-time)                                          %>% 
              group_by(case,ens_label,month)                                %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
...
maxsur_sens     = max_survi                                     %>% 
                  left_join(max_lhsur,by=c("case","ens_label"))
maxall_sens     = max_all                                       %>% 
                  left_join(max_lhall,by=c("case","ens_label"))

#2. group data into 3 different seasons: growing season (1,2,3,4),senescence season(5,6,7,8),
#  and dormant season(9,10,11,12) and calculate the mean value of each season, do decomposition for each season

gs_month = c("1","2","3","4")
ss_month = c("5","6","7","8")
ds_month = c("9","10","11","12")
gs_sur   =    survi_mmean                                                   %>% 
              filter(month %in% gs_month)                                   %>% 
              dplyr::select(all_of(c("case","ens_label",asses_vars)))       %>% 
              group_by(case,ens_label)                                      %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
              ungroup()
...
gs_lhall    = mmean_lh                                                      %>% 
              ...
              ungroup()
gs_lhsur    = mmean_lh                                                      %>% 
               filter(case_ens %in% survi_label & month %in% gs_month)      %>% 
               group_by(case,ens_label)                                     %>% 
               dplyr::select(-c(month,case_ens))                            %>% 
               summarize_all(mean,na.rm=TRUE)                               %>% 
               ungroup()
gsall_sens  = gs_all                                        %>% 
              left_join(gs_lhall, by=c("case","ens_label"))
gssur_sens  = gs_sur                                        %>% 
              left_join(gs_lhsur, by=c("case","ens_label"))

ss_sur   =    survi_mmean                                                   %>% 
              filter(month %in% ss_month)                                   %>% 
              dplyr::select(all_of(c("case","ens_label",asses_vars)))       %>% 
              group_by(case,ens_label)                                      %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
              ungroup()
...
ss_lhall    = mmean_lh                                                      %>% 
              ...
              ungroup()
ss_lhsur    = mmean_lh                                                      %>% 
               filter(case_ens %in% survi_label & month %in% ss_month)      %>% 
               group_by(case,ens_label)                                     %>% 
               dplyr::select(-c(month,case_ens))                            %>% 
               summarize_all(mean,na.rm=TRUE)                               %>% 
               ungroup()
ssall_sens  = ss_all                                        %>% 
              left_join(ss_lhall, by=c("case","ens_label"))
sssur_sens  = ss_sur                                        %>% 
              left_join(ss_lhsur, by=c("case","ens_label"))

ds_sur   =    survi_mmean                                                   %>% 
              filter(month %in% ds_month)                                   %>% 
              dplyr::select(all_of(c("case","ens_label",asses_vars)))       %>% 
              group_by(case,ens_label)                                      %>% 
              summarize_all(mean,na.rm=TRUE)                                %>% 
              ungroup()
...
ds_lhall    = mmean_lh                                                      %>% 
              ...
              ungroup()
ds_lhsur    = mmean_lh                                                      %>% 
               filter(case_ens %in% survi_label & month %in% ds_month)      %>% 
               group_by(case,ens_label)                                     %>% 
               dplyr::select(-c(month,case_ens))                            %>% 
               summarize_all(mean,na.rm=TRUE)                               %>% 
               ungroup()
dsall_sens  = ds_all                                        %>% 
              left_join(ds_lhall, by=c("case","ens_label"))
dssur_sens  = ds_sur                                        %>% 
              left_join(ds_lhsur, by=c("case","ens_label"))

asses_vars = c(asses_vars,"eflx_lh_tot")
...
maxsur_sens  = maxsur_sens                                        %>% 
               left_join(param_survi,by=c("case","ens_label"))
maxall_sens  = maxall_sens                                        %>% 
               left_join(param_all,by=c("case","ens_label"))
gssur_sens   = gssur_sens                                        %>% 
               left_join(param_survi,by=c("case","ens_label"))   %>% 
               mutate(co2_trt = ifelse(case %in% c(1,2,3), "low","high")
                     ,allom_grp = case_when(case %in% c(1,4) ~ "AVBA"
                                           ,case %in% c(2,5) ~ "BRDI"
                                           ,case %in% c(3,6) ~ "GENL"))

gsall_sens   = gsall_sens                                        %>% 
               left_join(param_all,by=c("case","ens_label"))
sssur_sens   = sssur_sens                                        %>% 
               left_join(param_survi,by=c("case","ens_label"))   %>% 
               mutate(co2_trt = ifelse(case %in% c(1,2,3), "low","high")
                     ,allom_grp = case_when(case %in% c(1,4) ~ "AVBA"
                                           ,case %in% c(2,5) ~ "BRDI"
...
ssall_sens   = ssall_sens                                        %>% 
               left_join(param_all,by=c("case","ens_label"))
dssur_sens   = dssur_sens                                        %>% 
               left_join(param_survi,by=c("case","ens_label"))   %>% 
               mutate(co2_trt = ifelse(case %in% c(1,2,3), "low","high")
                     ,allom_grp = case_when(case %in% c(1,4) ~ "AVBA"
                                           ,case %in% c(2,5) ~ "BRDI"
...
dsall_sens   = dsall_sens                                        %>% 
               left_join(param_all,by=c("case","ens_label"))

##3. mean value of all ensembles and only survived ensembles
lhsur_mean  = hlm1d                                         %>% 
              mutate(case_ens=paste0(case,"_",ens_label))   %>% 
              filter(case_ens %in% survi_label)             %>% 
              group_by(case_ens)                            %>% 
              summarize(eflx_lh_tot = mean(eflx_lh_tot))    %>% 
              ungroup()         
survi_mean    = survi_mean                           %>% 
                left_join(lhsur_mean,by="case_ens")  %>% 
                left_join(param_survi,by=c("case","ens_label","case_ens")) %>% 
                mutate(co2_trt = ifelse(case %in% c(1,2,3), "low","high")
                     ,allom_grp = case_when(case %in% c(1,4) ~ "AVBA"
                                           ,case %in% c(2,5) ~ "BRDI"
                                           ,case %in% c(3,6) ~ "GENL"))
...
all_mean     = all_mean                            %>% 
               left_join(lh_mean,by="case_ens")    %>% 
               left_join(param_all,by=c("case","ens_label","case_ens")) %>% 
               mutate(co2_trt = ifelse(case %in% c(1,2,3), "low","high")
                     ,allom_grp = case_when(case %in% c(1,4) ~ "AVBA"
                                           ,case %in% c(2,5) ~ "BRDI"
                                           ,case %in% c(3,6) ~ "GENL"))
...
```{r,label="parameter-variable-correlations",message=FALSE,fig.height=12, fig.width=14}

cor_survi   = survi_mean
cor_allens  = all_mean
cor_survi   = cor_survi[,c(1,7,11:15,17:21,23:40)] 
cor_survi   = cor_survi %>% filter(case %in% c(4,5,6))
cor_allens  = cor_allens[,c(1,7,11:15,17:21,23:40)]
cor_allens  = cor_allens %>% filter(case %in% c(4,5,6))
               
loop_case = which(case_names %in% unique(cor_survi$case))

for(c in loop_case){
  case_now  = case_names[c]
  desc_now  = substr(case_desc[c],1,4)
  survi_now = cor_survi                 %>% 
              filter(case == case_now)  %>% 
              dplyr::select(-case)
  all_now   = cor_allens                %>% 
  ...
  p_sur             = cor.mtest(survi_now,conf.level=0.95, method="pearson")$p
  corr_allens       = cor(all_now)
  p_all             = cor.mtest(all_now,conf.level=0.95, method="pearson")$p
  
  cor_nams          = c("GPP","$ Mort[Cstar]","$ Mort[Hydro]","$ Mort[Term]",
                        "$ Mort[Can]","$ Mort[Und]","AGB","LAI","LH",
                        "$ Leaf[Dia]","$ Leaf[Tunovr]","$ SLA[top]","$ Vc[max]","$ BB[incpt]",
                        "$ BB[slope]","LNC","$ Recruit[DBH]", "$ Recruit[Alloc]",
                        "$ Recruit[hgt]", "$ C[Storage]","$ SWC[Phen]","smpsc",
                        "smpso","$ Root[a]","$ Root[b]","$ Mort[hyd-scala]",
                        "$ Mort[cst-scala]","$ Decomp[max]","$ Respir[grow]")
  colnames(corr_surviens) = cor_nams
  rownames(corr_surviens) = cor_nams
  colnames(corr_allens) = cor_nams
  rownames(corr_allens) = cor_nams
  colnames(p_sur) = cor_nams
  rownames(p_sur) = cor_nams
  colnames(p_all) = cor_nams
  rownames(p_all) = cor_nams
  
  f_output = paste0(desc_now,"_","allmean_correlation.png")
  png(file=file.path(plot_main,f_output),
  ...
  f_output=paste0(desc_now,"_","survimean-correlation.png")
  png(file=file.path(plot_main,f_output),
     width=6.5,height=6.5,units="in",res=600)
...
lst_col = ncol(sens_main)

sens_plot = list()
sens_df   = all_mean #to select doing variance decompose for all or survived ensembles
type      = "ALLMean"
...
for(c in sequence(ncase)){
  ...
  for(p in sequence(n_param)){
    p_now   = pars[p] 
    df_now  = sens_now %>% dplyr::select(p_now,var_now)
  ...
  sens_main[v,lst_col]    = desc_now  
  }else{
  v_idx = (c-1)*n_var   + v
  sens_main[v_idx,p] = var(moddf$pred)/var(moddf$y)
  sens_main[v_idx,lst_col]    = desc_now 
       }#end if else
...
## overall with survived ensembles

pars_all = c(pars,"allom_grp")
npar_all = length(pars_all)
sensall_main = matrix( data=NA_real_
                     , nrow=n_var
                     , ncol=npar_all
                     , dimnames = list(asses_vars,pars_all))
sens_now = survi_mean
type="SurviMean"
sens_plot=list()
for(v in sequence(n_var)){
  var_now = asses_vars[v]
  for(p in sequence(npar_all)){
    p_now   = pars_all[p] 
    df_now  = sens_now %>% dplyr::select(p_now,var_now)
    df_sort = df_now[do.call(order, df_now[, p_now]), ]
    x       = df_sort[[p_now]]
  ...
  sens_plot[[var_now]][[p_now]]=gg_now
    }#end of if(var_plot)
  
  sensall_main[v,p]     = var(moddf$pred)/var(moddf$y)
  }
}


## overall by season
sea_df   = c("gssur_sens","sssur_sens", "dssur_sens")  
sea_desc = c("Growth", "Senescence", "Emergence")
sea_num  = c(1L,2L,3L)
names(sea_desc)=c("1","2","3")
nsea     = length(sea_df)
sensea_main = matrix( data=NA_real_
                     , nrow=n_var*nsea
                     , ncol=npar_all+1
                     , dimnames = list(rep(asses_vars,times=nsea),c(pars_all, "season")))
lst_col     = ncol(sensea_main)
sens_plot=list()
for(s in sequence(nsea)){
  df       = sea_df[s]
  type     = sea_num[s]
  sens_now = get(df)
...
for(v in sequence(n_var)){
  ...
  sens_plot[[sens_now]][[var_now]][[p_now]]=gg_now
    }#end of if(var_plot)
  
   if(s==1){
  sensea_main[v,p]          = var(moddf$pred)/var(moddf$y)
  sensea_main[v,lst_col]    = type  
  }else{
  v_idx = (s-1)*n_var   + v
  sensea_main[v_idx,p] = var(moddf$pred)/var(moddf$y)
  sensea_main[v_idx,lst_col] = type 
       }#end if else
  }
}
}
...
sens_now = survi_mean
type = "whole-SurMean"
sensall_min = matrix( data=NA_real_
                    ,nrow=n_var
                    ,ncol=npar_all
                    ,dimnames = list(asses_vars,pars_all))
sens_plot=list()
 for(v in sequence(n_var)){
  ...
  for(p in sequence(npar_all)){
    ...
    x       = sens_now[[p_now]]
    y_rsd   = y
    others  = pars_all[pars_all!=p_now]
    n_other = length(others)
    for(t in sequence(n_other)){
      other_var = others[t]
      x_other   = sens_now[[other_var]]
...
sensea_min = matrix( data=NA_real_
                    ,nrow=n_var*nsea
                    ,ncol=npar_all+1
                    ,dimnames = list(rep(asses_vars,times=nsea),c(pars_all,"season")))

lst_col = ncol(sensea_min)
sens_plot=list()
for(s in sequence(nsea)){
  df       = sea_df[s]
  type     = sea_num[s]
 ...
 for(v in sequence(n_var)){
  ...
  for(p in sequence(npar_all)){
    ...
    var_min   = var(plotdf$pred)/var(plotdf$orig_y)
    if(s==1){
     sensea_min[v,p] = var_min 
     sensea_min[v,lst_col]=type
    }else{
     v_idx = (s-1)*n_var + v
     sensea_min[v_idx,p]  = var_min
     sensea_min[v_idx,lst_col] = type
    } #end if else
   if(var_plot){
    gg_now = ggplot(data=plotdf,aes(x,y))
    gg_now = gg_now + geom_point()
...
par_label            = cor_nams[10:29]
par_label            = gsub(pattern="\\$ ", replacement="",x=par_label)

sens_decom           = sens_decom                                                %>% 
                       mutate( var_decomp = round(as.numeric(var_decomp)*100,1)
                             , label      = rep(par_label,time=n_var*ncase*2)) 

slec_var             = c("fates_gpp","fates_lai","fates_vegc_aboveground","eflx_lh_tot",
                         "fates_mortality_cstarv","fates_mortality_hydraulic")
decom_part           = sens_decom                     %>% 
                       filter(model_var %in% slec_var & type=="main")
  
var_label            = c("Growth primary productivity"
                        ,"Leaf area index"
                        ,"Latent heat flux"
                       # ,"Canopy mortality"
...
for(c in sequence(ncase)){
  desc_now   = case_desc[c]
  decomp_now = decom_part %>% filter(case==desc_now)
  
  gg_now = ggplot(decomp_now,aes(var_decomp,label)) 
  #gg_now = gg_now + geom_linerange(aes(xmin=0,xmax=var_decomp,colour=type)
  #                                ,size=4, alpha=0.7)#,position = position_dodge(width = 0.2))
  gg_now = gg_now + geom_linerange(aes(xmin=0,xmax=var_decomp),size=5)
  #gg_now = gg_now + scale_colour_manual(values = ens_colour[c(1,3)])
  ...
  gg_now = gg_now + theme_bw( base_size = gg_ptsz*1.5
...
## overall
type = "SurviMean"
var                  = rownames(sensall_main)
sensall_madf         = as_tibble(sensall_main)
sensall_madf$model_var  = var
sensall_madf$type       = "main"

var                  = rownames(sensall_min)
sensall_midf         = as_tibble(sensall_min)
sensall_midf$model_var  = var
sensall_midf$type       = "min"

sensall_decom           = rbind(sensall_madf,sensall_midf)
sensall_decom           = sensall_decom                                        %>%  
                          pivot_longer(cols = turb_diameter:allom_grp
                                   ,names_to  = "parameter"
                                   ,values_to = "var_decomp")   

par_label            = c(cor_nams[10:29],"Allometry")
par_label            = gsub(pattern="\\$ ", replacement="",x=par_label)

sensall_decom        = sensall_decom                                                %>% 
                       mutate( var_decomp = round(as.numeric(var_decomp)*100,1)
                             , label      = rep(par_label,time=n_var*2)) 

slec_var             = c("fates_gpp","fates_lai","fates_vegc_aboveground","eflx_lh_tot")
decomall_part           = sensall_decom                                                          %>% 
                          filter(model_var %in% slec_var & type=="main" )#& parameter!="co2_trt")
  
loop_list   = which(hlm1dvar$vnam %in% slec_var)
decomp_plot=list()
for(d in loop_list){
   h_vnam   = hlm1dvar$vnam [d] 
   h_desc   = hlm1dvar$desc [d]
   h_short  = hlm1dvar$short[d]
   h_unit   = hlm1dvar$unit [d]
   h_legend = v == 1
   cat0("   - ",h_desc,".")
   decomp_now = decomall_part %>% filter(model_var==h_vnam)
   decomp_now = decomp_now %>% arrange(var_decomp)
   decomp_now = decomp_now %>% mutate(label=factor(label,levels=unique(label))) %>% filter(var_decomp>1)
  ...
  #gg_now = gg_now + scale_colour_manual(values = ens_colour[c(1,3)])
  #gg_now = gg_now + facet_wrap(.~ model_var,labeller=labeller(model_var=var_label))
  #gg_now = gg_now + ggtitle(parse(text=h_short))
  gg_now = gg_now + ggtitle(h_desc)
  gg_now = gg_now + xlab("Variance explained (%)")
  gg_now = gg_now + ylab(element_blank())
  gg_now = gg_now + scale_y_discrete(labels = scales::label_parse())
  gg_now = gg_now + theme_bw( base_size = gg_ptsz*1.5
...
decomp_plot[[h_vnam]] = gg_now
}


decomp_plot_reorder = list(decomp_plot$fates_gpp,decomp_plot$fates_lai,decomp_plot$fates_vegc_aboveground,decomp_plot$eflx_lh_tot)
 lai_plot = decomp_plot$fates_lai
 lh_plot  = decomp_plot$eflx_lh_tot
gg_patch = wrap_plots(decomp_plot_reorder)
gg_patch = gg_patch + guide_area() + plot_layout( guides="collect",ncol=2)
gg_patch = gg_patch + plot_annotation(tag_level = "a",title = "")
  for (d in sequence(ndevice)){
    f_output = paste0(type,"-variance-decomp.",gg_device[d])
    dummy    = ggsave( filename = f_output
                     , plot     = gg_patch
                     , device   = gg_device[d]
                     , path     = plot_main
...
type="bysea-SurMean"
var                  = rownames(sensea_main)
sensea_madf          = as_tibble(sensea_main)
sensea_madf$model_var  = var
sensea_madf$type       = "main"

var                  = rownames(sensea_min)
sensea_midf          = as_tibble(sensea_min)
sensea_midf$model_var  = var
sensea_midf$type       = "min"

sensea_decom           = rbind(sensea_madf,sensea_midf)
sensea_decom           = sensea_decom                                        %>%  
                          pivot_longer(cols = turb_diameter:allom_grp
                                   ,names_to  = "parameter"
                                   ,values_to = "var_decomp")   
par_label            = c(cor_nams[10:29], "Allometry")
...
sensea_decom        = sensea_decom                                                %>% 
                       mutate( var_decomp = round(as.numeric(var_decomp)*100,1)
                             , label      = rep(par_label,time=n_var*nsea*2)) 

slec_var             = c("fates_gpp","fates_lai","fates_vegc_aboveground","eflx_lh_tot")
decomsea_part           = sensea_decom                                                          %>% 
                          filter(model_var %in% slec_var & type=="main" )#& parameter!="co2_trt")
...
for(d in loop_list){
   ...
   h_legend = v == 1
   cat0("   - ",h_desc,".")
   decomp_now = decomsea_part %>% filter(model_var==h_vnam)
   decomp_now = decomp_now %>% mutate(label=factor(label,levels=unique(label))) %>% filter(var_decomp>5)
   gg_now = ggplot(decomp_now,aes(var_decomp,label)) 
   gg_now = gg_now + facet_wrap(.~season, labeller=labeller(season=sea_desc))
   gg_now = gg_now + geom_linerange(aes(xmin=0,xmax=var_decomp),size=5)
   gg_now = gg_now + ggtitle(h_desc)
   gg_now = gg_now + xlab("Variance explained (%)")
   gg_now = gg_now + ylab(element_blank())
...
## radial plot
test = decomp_now %>% filter(model_var=="fates_gpp")
test = test %>% arrange(desc(var_decomp))
test = test %>% mutate(label=factor(label,levels=unique(label)))

xgrid= c(0, 5, 10, 15, 20,25,30)
p = ggplot(test, aes(label)) +
  geom_hline(yintercept=xgrid, colour="grey80", size=0.6, linetype="dotted") +
  geom_segment(data=data.frame(x=unique(test$label), y=0, yend=max(xgrid)),
               aes(x=x, xend=x, y, yend=yend), colour='grey30', size=0.6,linetype="dotted",
               position=position_nudge(x=0.5)) +
  geom_segment(data=data.frame(x=0.5), x=0.5, xend=0.5, y=0, yend=max(xgrid), colour='grey30', size=0.6,linetype="dotted") +
  #geom_col(aes(y=var_decomp), width=0.5, position="identity") +
  geom_col(aes(y=var_decomp, fill=type), width=0.5, position="identity") +
  #geom_errorbar(aes(ymin=var_decomp - conf, ymax=value + conf, colour=key), 
  #              position=position_dodge(0.2), width=0.2) +
  geom_text(aes(label=label,y=max(xgrid)), parse=TRUE, size=4, colour="grey30") +
  annotate(y=xgrid, x=0, label=paste0(xgrid,"%"), geom="text", size=4,colour="grey30") +
  coord_polar(start=-2*pi/22 * 0.5) + 
  scale_y_continuous(limits=c(-20, max(xgrid)), expand=c(0,0)) +
...
```{r,label="visualize-parameter-effects"}
...
lhsur_mmean  = hlm1d                                         %>% 
               ...
               mutate(month    = month(time)
                     ,case_ens = paste0(case,"_",ens_label)) %>%
               filter(case_ens %in% survi_label)             %>% 
               dplyr::select(-c(time,case_ens))              %>% 
               group_by(case,ens_label,month)                %>% 
               summarize_all(mean,na.rm=TRUE)                %>% 
               ungroup()
plot_pareff = survi_mmean                                    %>% 
              left_join(lhsur_mmean
                       ,by=c("case","ens_label","month"))                    %>% 
              group_by(case,ens_label)                                       %>% 
              mutate(lh_peakT  = month[which.max(eflx_lh_tot)]
                    ,lh_peak   = max(eflx_lh_tot)     
                    ,lai_peakT = month[which.max(fates_lai)]
                    ,lai_peak  = max(fates_lai)
                    ,gpp_peakT = month[which.max(fates_gpp)]
                    ,gpp_peak  = max(fates_gpp)
                    ,agb_peakT = month[which.max(fates_vegc_aboveground)]
                    ,agb_peak  = max(fates_vegc_aboveground))                %>% 
             ungroup()


plot_pareff = plot_pareff                                           %>% 
              group_by(case,ens_label)                              %>% 
              mutate(lai_grp = cut(lai_peak
                                  ,breaks = c(-Inf,1.5,2.5,Inf)
                                  ,labels = c("LAI[0-1.5~m^2~m^{-2}]"
                                             ,"LAI[1.5-2.5~m^2~m^{-2}]"
                                             ,"LAI[2.5-Inf~m^2~m^{-2}]"))
                    ,gpp_grp = cut(gpp_peak
                                  ,breaks = c(-Inf,5,7.5,Inf)
                                  ,labels = c("GPP[0-5~gC~m^{-2}~Day^{-1}]"
                                             ,"GPP[5-7.5~gC~m^{-2}~Day^{-1}]"
                                             ,"GPP[7.5-Inf~gC~m^{-2}~Day^{-1}]"))
                    ,lh_grp  = cut(lh_peak
                                  ,breaks = c(-Inf,60,75,Inf)
                                  ,labels = c("lambda*E[0-60~W~m^{-2}]"
                                             ,"lambda*E[60-75~W~m^{-2}]"
                                             ,"lambda*E[75-Inf~W~m^{-2}]"))
                    ,agb_grp = cut(agb_peak
                                  ,breaks = c(-Inf,0.15,0.25,Inf)
                                  ,labels = c("AGB[0-0.15~kgC~m^{-2}]"
                                             ,"AGB[0.15-0.25~kgC~m^{-2}]"
                                             ,"AGB[0.25-Inf~kgC~m^{-2}]")))    %>% 
              ungroup()
              
              
 only_lh = plot_pareff                                                %>% 
               dplyr::select(case,ens_label,lh_peak,lh_peakT,lh_grp)  %>% 
               left_join(param_survi,by=c("case","ens_label"))        %>% 
               distinct()
 only_lh = only_lh                                                    %>% 
           arrange(lh_peakT)                                          %>% 
           mutate(lh_peakT = factor(lh_peakT
                                  ,labels=c("March","April","May")))

gg_now = ggplot(only_lh,aes(phen_drought_threshold,recruit_height_min,colour=lh_grp)) 
gg_now = gg_now +  geom_point(aes(shape=lh_peakT)) 
gg_now = gg_now +  facet_wrap(.~case,labeller=labeller(case=case_desc)) 
#gg_now = gg_now +  geom_boxplot()
gg_now = gg_now +  scale_colour_manual(labels = parse_format()
                                      ,values=ens_colour[c(1:3)]
                                      ,name="Peak value")
gg_now = gg_now +  scale_shape_manual(values=c(13,16:17),name="Peak month")
gg_now = gg_now +  labs(x=expression(Soil~water~content[Drought-Decid])
                       ,y=expression(Recruitment[Height-min]~"(m)"))
...
p_output = paste0("LH-param-effects.",gg_device)
...
gg_now = ggplot(only_lh,aes(phen_drough,recruit_height_min,colour=lh_grp)) 
gg_now = gg_now +  geom_point(aes(shape=lh_peakT)) 
gg_now = gg_now +  facet_wrap(.~case,labeller=labeller(case=case_desc)) 
#gg_now = gg_now +  geom_boxplot()
gg_now = gg_now +  scale_colour_manual(labels = parse_format()
...
#ens_filt    = ens_filt                                                                              %>% 
#              mutate(eflx_lh_tot_qlwr = if_else(month==3,eflx_lh_tot_qlwr *lw.dec,eflx_lh_tot_qlwr)
#                    ,eflx_lh_tot_qupr = if_else(month==3,eflx_lh_tot_qupr*hi.add,eflx_lh_tot_qupr)
#                    ,fates_gpp_qlwr   = if_else(month==3,fates_gpp_qlwr*lw.dec,fates_gpp_qlwr)
#                    ,fates_gpp_qupr   = if_else(month==3,fates_gpp_qupr*hi.add,fates_gpp_qupr)
#                    )
...
ens_filt    = ens_filt                        %>% 
              #filter(!month== 3)                         %>% #Both LH and GPP have narrow range in March
              mutate(month_keep = if_else(                   #we exclude this month to be more relaxing
...
ens_keep  = unique(ens_filt$case_ens)  


### using RMSE
model_rmse  = function(model, data){
...
agb_filt   = ens_filt                                   %>% 
             group_by(case,ens_label)                   %>% 
             summarize(agb_rmse = model_rmse(model=fates_vegc_aboveground_mod,
                                             data = fates_vegc_aboveground))
...
```{r,label="kept-members-seasonal-dynamic",message=FALSE,results='hide'}
...
for (h in keep_loop){
   ...
   gg_now = gg_now + geom_line(aes_string(x="month",y=h_vnam,group="case_ens",colour="case")
                              ,lwd=0.5,alpha=0.8,show.legend=TRUE)
   ...
   gg_now = gg_now + labs(title="")
   gg_now = gg_now + scale_x_continuous( breaks = sequence(12))#end scale_x_continuous
   if(h_vnam %in% c("eflx_lh_tot","fsh")){gg_now = gg_now + xlab("Month of year")}else{gg_now = gg_now + xlab(element_blank())}
   gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
   gg_now = gg_now + theme_bw( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
   gg_now = gg_now + theme(axis.text.x        = element_text( size  = gg_ptsz
                                                           , margin = unit(rep(0.35,times=4),"cm"))#end element_text
  ...
  gg_keep_reorder = list(gg_keep$fates_gpp,gg_keep$elai,gg_keep$fates_vegc_aboveground,gg_keep$eflx_lh_tot,gg_keep$fsh)
  gg_patch = wrap_plots(gg_keep_reorder)
  gg_patch = gg_patch + guide_area() + plot_layout(guides="collect")
  gg_patch = gg_patch + plot_annotation(tag_levels = "a", title = "")
  
 # Save plots.
for (d in sequence(ndevice)){
    h_output = paste0("kept-ens-obs1.2-compare.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot     = gg_patch
                    , device   = gg_device[d]
                    , path     = secy_path
...
site       = sierra_amean  %>% mutate(case = "Seabloom-SierraFoothill")
amean_now  = keep_fates
amean_now$case = factor(amean_now$case,levels=unique(amean_now$case))
   
gg_now = ggplot(data=amean_now)
gg_now = gg_now + geom_line(aes(x=year,y=agb,group=case_ens,color=case)
                              ,lwd=1,alpha=0.8,show.legend=TRUE)
gg_now = gg_now + geom_line(data = site,aes(x=year,y=agb,color=case))
gg_now = gg_now + scale_colour_manual(name="",values=c(ens_colour[1:3],site_colour),
                                      labels = c(case_desc, "Seabloom-SierraFoothill"))
...
```{r,label="check-co2-effects"}

keep_delta = delta_co2                                     %>% 
             mutate(case = case_when(alom_grp=="AVBA" ~ 4
                                    ,alom_grp=="BRDI" ~ 5
                                    ,alom_grp=="GENL" ~ 6)
                  ,case_ens = paste0(case,"_",ens_label))  %>% 
            filter(case_ens %in% ens_keep)
keep_delta = keep_delta                              %>% 
             mutate(month = month(time))             %>% 
             dplyr::select(-c(time,alom_grp))        %>% 
             group_by(case,ens_label,case_ens,month) %>% 
             summarize_all(mean,na.rm=TRUE)

keepR_delta = deltaR_co2                                     %>% 
             mutate(case = case_when(alom_grp=="AVBA" ~ 4
                                    ,alom_grp=="BRDI" ~ 5
                                    ,alom_grp=="GENL" ~ 6)
                  ,case_ens = paste0(case,"_",ens_label))  %>% 
            filter(case_ens %in% ens_keep)
keepR_delta = keepR_delta                            %>% 
             mutate(month = month(time))             %>% 
             dplyr::select(-c(time,alom_grp))        %>% 
             group_by(case,ens_label,case_ens,month) %>% 
             summarize_all(mean,na.rm=TRUE)
...
for (h in delta_loop){
   ...
   delta_now        = keep_delta
   #mmean_now$case = factor(mmean_now$case,levels=unique(mmean_now$case))
   
   gg_now = ggplot(data=delta_now)
   gg_now = gg_now + facet_wrap(.~case,labeller=labeller(case=case_desc))
   gg_now = gg_now + geom_line(aes_string(x="month",y=h_vnam,colour="ens_label")
                              ,lwd=1,alpha=0.8,show.legend=TRUE)
   
   gg_now = gg_now + scale_colour_manual(name="",aesthetics="colour",values=top12)
   gg_now = gg_now + labs(title=h_desc)
   gg_now = gg_now + scale_x_continuous( breaks = sequence(12))#end scale_x_continuous
   gg_now = gg_now + xlab(element_blank())
   #gg_now = gg_now + ylab(desc.unit(desc=h_short,unit=untab[[h_unit]],dxpr=TRUE))
   ...
   # Save plots.
   for (d in sequence(ndevice)){
    h_output = paste0(h_vnam,"_co2DeltaKeep.",gg_device[d])
    dummy    = ggsave( filename = h_output
                    , plot      = gg_now
                    , device    = gg_device[d]
                    , path      = secy_path
...
```{r,label="agb-range-model-ob-compare",results='hide'}
...
for (h in one_loop){
   ...
   gg_now = gg_now + theme_grey( base_size = gg_ptsz, base_family = "Helvetica",base_line_size= 0.5,base_rect_size =0.5)
   gg_now = gg_now + theme( legend.position   = "right"
                          , legend.text=element_text(size=gg_ptsz*0.9)
                          , axis.text.x        = element_text( size   = gg_ptsz
                                                           , margin = unit(rep(0.35,times=4),"cm"))#end element_text
