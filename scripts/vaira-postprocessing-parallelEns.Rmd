...
var_both = var_both[! var_both %in% c("time","case","ens_label")]
...
site_mrange = merge(x=site_mlwr,y=merge(x=site_mean,y = site_mupr,by=c("month","ens_label","case")),
                    by=c("month","ens_label","case"))

#have to rename gpp and agb so names match for making seasonal dynamic plots
site_var = names(site_mrange)
...
for (h in emean_loop){
   ...
   obs              = site_mrange %>% dplyr::select(-case)
