...
deltaR_co2 = delta_co2                                                          %>% 
            ...
            mutate(fates_gpp                 = if_else(fates_gpp_400==0
                                                      ,0
                                                      ,(fates_gpp_400-fates_gpp_280)/fates_gpp_400)
                     ,elai                   = if_else(elai_400==0
                                                      ,0
                                                      ,(elai_400-elai_280)/elai_400)
                     ,fates_vegc_aboveground = if_else(fates_vegc_aboveground_400==0
                     ,0
                     ,(fates_vegc_aboveground_400-fates_vegc_aboveground_280)/fates_vegc_aboveground_400) 
                     ,fsh                    = if_else(fsh_400==0
                                                      ,0
                                                     ,(fsh_400-fsh_280)/fsh_400)
                    ,eflx_lh_tot            = (if_else(eflx_lh_tot_400==0
                                                      ,0
...
cor_survi   = gssur_sens
cor_allens  = gsall_sens
type        = "growing"
cor_survi   = cor_survi[c(1,3:13,15:32)] 
cor_allens  = cor_allens[c(1,3:13,15:32)]
...
for(c in sequence(ncase)){
  ...
  cor_nams          = c("GPP","$ Mort[Cstar]","$ Mort[Hydro]","$ Mort[Term]",
                        ...
                        "$ Leaf[Dia]","$ Leaf[Tunovr]","SLA","vcmax","$ BB[incpt]",
...
sens_plot = list()
sens_df   = sssur_sens #to select doing variance decompose for all or survived ensembles
type      = "senescence"
for(c in sequence(ncase)){
  case_now  = case_names[c]
  desc_now  = case_desc[c]
...
```{r,"agb-range-model-ob-compare",results='hide'}
