...
#case_name  = c("fire1PFT_30S3C_CLM_FATES","fire1PFT_40S30C_CLM_FATES", "fire1PFT_40S30Csrt_CLM_FATES",
               #"fire1PFT_52S10C_CLM_FATES", "fire1PFT_52S10Csrt_CLM_FATES","fire1PFT_evghi_CLM_FATES")     # Simulation path
case_name = c("sapwood-allom-test_CLM_FATES")
case_desc  = "sapwood allometry diagnose" # Simulation description
#case_label = c("30S3C", "40S30C","40S30Csrt", "52S10C", "52S10Csrt","30S13C")
...
tstampa    = "01/01/2000"# "01/01/2004"         # First time (MM/DD/YYYY)
tstampz    = "12/01/2020" # "12/01/2019"         # Last time (MM/DD/YYYY)

readtime   = TRUE  # use this logic flag to indicate whether we are reading files one by one, used in unit conversion for monthly                     # hlm1d variables, see line 816-818
...
# as 2 simulations have exact same outputs, size and age class, so we just use one to initialize each
# List of age classes
...
if (  all(c("FATES_LAI_USTORY_SZ","FATES_LAI_CANOPY_SZ") %in% nc_bydbh)
   && (! "FATES_LAI_SZ" %in% nc_bydbh) ){
   nc_bydbh    = unique(c(nc_bydbh,"FATES_LAI_SZ"))
   laidbh_last = TRUE
}else if (  all(c("FATES_LAI_USTORY_SZPF","FATES_LAI_CANOPY_SZPF") %in% nc_bydbh)
   && (! "FATES_LAI_SZPF" %in% nc_bydbh) ){
   nc_bydbh    = unique(c(nc_bydbh,"FATES_LAI_SZPF"))
   laidbh_last = TRUE
}else{
   laidbh_last = FALSE
...
if (  all(c("FATES_LAI_USTORY_SZPF","FATES_LAI_CANOPY_SZPF") %in% nc_bypft)
   && (! "FATES_LAI_SZPF" %in% nc_bypft) ){
   nc_bypft    = unique(c(nc_bypft,"FATES_LAI_SZPF"))
   laipft_last = TRUE
}else{
   laipft_last = FALSE
...
for (n in sequence(ncase)){
   ...
   for (a in sequence(nbyage)){
      ...
      byage[[liname]][[f_vnam]] = matrix(data=NA_real_,nrow=ntstamp,ncol=nages,dimnames=list(NULL,agekeys))
...
for(n in sequence(ncase)){
   ...
   for (d in sequence(nbydbh)){
      ...
      f               = match(nc_pref,fatesvar$vnam)
      f_vnam          = fatesvar$vnam[f]
      bydbh[[liname]][[f_vnam]] = matrix(data=NA_real_,nrow=ntstamp,ncol=ndbhs,dimnames=list(NULL,dbhkeys))
...
case = cnames[n]
slayer_now = tibble( zsoi   = c(unlist(ncvar_get(nc=nc_conn, varid='ZSOI'))) #,unlist(ncvar_get(nc=nc_connfull,varid='ZSOI' )))
               , dzsoi  = c(unlist(ncvar_get(nc=nc_conn,varid='DZSOI' ))) #, unlist(ncvar_get(nc=nc_connfull,varid='DZSOI' )))
               , bsw    = c(unlist(ncvar_get(nc=nc_conn,varid='BSW' ))) #, unlist(ncvar_get(nc=nc_connfull,varid='BSW' )))
               , hksat  = c(unlist(ncvar_get(nc=nc_conn,varid='HKSAT' ))) #, unlist(ncvar_get(nc=nc_connfull,varid='HKSAT' )))
               , sucsat = c(unlist(ncvar_get(nc=nc_conn,varid='SUCSAT' ))) #, unlist(ncvar_get(nc=nc_connfull,varid='SUCSAT' )))
               , watsat = c(unlist(ncvar_get(nc=nc_conn,varid='WATSAT' ))) #, unlist(ncvar_get(nc=nc_connfull,varid='WATSAT' )))
...
#      }
   ...
   monthvar = c("qdirect_throughfall","qdrai","qdrip",
                "qevtr","	qintr","qover","qsoil","qvege",
                "qvegt","rain")
   ...
   if (laidbh_last){
      bydbh[[case]]$fates_lai[w,] = bydbh[[case]]$fates_lai_canopy[w,] + bydbh[[case]]$fates_lai_ustory[w,]
   }#end if (lai.last)
   ...
   for (p in sequence(nbypft-laipft_last)){
     ...
     if (laipft_last){
      bypft[[case]]$fates_lai[w,] = bypft[[case]]$fates_lai_canopy[w,] + bypft[[case]]$fates_lai_ustory[w,]
   }#end if (laipft_last)
   ...
   for (v in sequence(nhlm1d-etr_last-er_last)){
      ...
      h_add0              = eval(parse(text=hlm1dvar$add0[h]))
       if(readtime & h_vnam %in% monthvar){
       h_mult = cmon.sec
       }else{h_mult              = eval(parse(text=hlm1dvar$mult[h]))  }
...
#dbh_meltnz <- dbh_melt %>% filter(if_any(where(is.numeric), ~.x != 0))
#dbh_nz <- as.character(unique(dbh_meltnz$dbh))
#f <- match(dbh_nz, dbhkeys)
#dbhs_nz <- sort(dbhs[f])
#ndbhs_nz <- length(dbh_nz)
#dbhkeys_nz <- sprintf("dbh_%3.3i", dbhs_nz)
#dbhlabs_nz <- c(paste0("paste(",dbhs_nz[-ndbhs_nz], "-", dbhs_nz[-1], ")"),
#                paste0("paste(", dbhs_nz[ndbhs_nz], "-", "-infinity)"))
#dbhlabs_nz <- parse(text=dbhlabs_nz)
...
emean_soi10 = bysoi                                                       %>% 
              ...
              group_by(time, case)                                        %>% 
              #mutate(sdzsoi = sum(dzsoi, na.rm=TRUE))                     %>% 
              #mutate(dzsoi = dzsoi/sdzsoi)                                %>% 
              #mutate(h2osoi = h2osoi*dzsoi)                               %>% 
              #select(!c(sdzsoi, dzsoi) )                                  %>% 
